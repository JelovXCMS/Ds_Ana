...
int MC_EffCalLoop_DpTest(int isPbPb=0, int PNPrompt = 0, int DsChannel=10  ){
	...
	TString GenWeight_Pythia="weight";
	TString GenWeight_FONLL="weight";
	TString GenWeight_D0Data="weight";
	...
	if(isPbPb){
		...
		GenWeight_Pythia="weight*Ncoll";
		GenWeight_FONLL="weight*Ncoll";
		GenWeight_D0Data="weight*Ncoll";
	...
	if(isPbPb==0 && PNPrompt==0 && DsChannel==10){MCFile="/scratch/halstead/p/peng43/Ds_phikkpi/temp/DsMinTree_Dp_pp_mc_pt2.root";  }
...
// more scan
  ...
  TTree *t_DsMass_AllDauPtScan[nbin_pt][nbin_DauPtScan];
	...
	int nbin_DsMass=100;
	double binLow_DsMass=1.75;
	double binHi_DsMass=2.15;
  ...
  cout<<"check4"<<endl;
	...
	for(Long64_t ientry=0 ; ientry<nentries_Reco; ientry++){
		...
		if(isPbPb){
			...
			D0DataWeight=weight*DsMCReco.Ncoll;
			FONLLWeight=weight*DsMCReco.Ncoll;
			PythiaWeight=weight*DsMCReco.Ncoll;
		}
