...
//  TLatex* texSimPbPb = new TLatex(0.88,tex_upperY, "530 #mub^{-1} (5.02 TeV PbPb)");
 ...
 // 3. fit
	...
	RooRealVar DsWidth1_MC("DsWidth1_MC","DsWidth1_MC",0.0107,0.006,0.03);
	RooRealVar DsWidth2_MC("DsWidth2_MC","DsWidth2_MC",0.0058,0.0001,0.01);
	RooRealVar DsGaus1Fr_MC("DsGaus1Fr_MC","DsGaus1Fr_MC",0.8,1e-4,1.0);
...
*/
  ...
  RooPlot *massframe_pull=new RooPlot("massframe_pull","Dmass",Dmass,DsDataFitRangeLow,DsDataFitRangeHigh,nbin_DmassDraw);
	...
	massframe_pull->SetMaximum(3);
	massframe_pull->SetMinimum(-3);
  ...
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,"|y|<1.0"); shiftY-=oneshift;
	...
	TLegend *le_nopull=new TLegend(0.58,0.39,0.8,0.59);
	...
	SavePlotDirs(c_roofit_Data[count_c_rootfit],Form("%s_Data_unbinfit_nopull_pt%.0fto%.0f_%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data() ),savedirs );
  ...
  massframe_pull->Draw();
	...
	SavePlotDirs(c_roofit_Data_withpull[count_c_rootfit],Form("%s_Data_unbinfit_pt%.0fto%.0f_%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data() ),savedirs );
  ...
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("RooFitChi2 pvale : %.2f",pvalue_RooFit)); shiftY-=oneshift;
	...
	SavePlotDirs(c_roofit_Data_withpull[count_c_rootfit],Form("%s_Data_unbinfit_pt%.0fto%.0f_%s%s_withGOF",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data() ),savedirs );
...
*/
  ...
  TLatex* texCmsPre1 = new TLatex(0.15,tex_upperY1, "#scale[1.25]{CMS} Preliminary");
  texCmsPre1->SetNDC();
  texCmsPre1->SetTextAlign(12);
  texCmsPre1->SetTextSize(0.035);
  texCmsPre1->SetTextFont(42);

  TLatex* texCmsSim1 = new TLatex(0.15,tex_upperY1, "#scale[1.25]{CMS} Simulations");
  texCmsSim1->SetNDC();
  texCmsSim1->SetTextAlign(12);
  texCmsSim1->SetTextSize(0.035);
  texCmsSim1->SetTextFont(42);


  // TLatex* texColPbPb = new TLatex(0.88,tex_upperY, "PbPb #sqrt{s_{NN}} = 5.02 TeV");
  TLatex* texColPbPb1 = new TLatex(0.97,tex_upperY1, "530 #mub^{-1} (5.02 TeV PbPb)");
  texColPbPb1->SetNDC();
  texColPbPb1->SetTextAlign(32);
  texColPbPb1->SetTextSize(0.035);
  texColPbPb1->SetTextFont(42);

  // TLatex* texColpp = new TLatex(0.88,tex_upperY, "pp #sqrt{s} = 5.02 TeV");
  TLatex* texColpp1 = new TLatex(0.97,tex_upperY1, "27.4 pb^{-1} (5.02 TeV pp )");
  texColpp1->SetNDC();
  texColpp1->SetTextAlign(32);
  texColpp1->SetTextSize(0.035);
...
int SignalFit_fixShapeCutScanTest(int isPbPb=0, int ibin_Dpt=7,int doMCstudySet=0, int doBkgVariationSet=0, int doSigVariationSet=0, int doCutScanSet=0){
