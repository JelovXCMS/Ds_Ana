...
// TCanvas *c_roofit = new TCanvas("c_roofit","c_roofit",800,800);
	...
	double DsDataFitRangeLow =1.78;
	double DsDataFitRangeHigh = 2.15;
  ...
  double D0FloatWidthV_N=0;
  double DsMassMeanV_N=0;
  double D0MassMeanV_N=0;
...
//  TLatex* texSimPbPb = new TLatex(0.88,tex_upperY, "530 #mub^{-1} (5.02 TeV PbPb)");
  ...
  savedirs.push_back("SignalFit_wide");
 ...
 // 3. fit
  ...
  // SavePlotDirs(c_roofit_MC[count_c_rootfit],Form("%s_MC_unbinfit_pt%.0fto%.0f_%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data() ),{"SignalFit",str_PbPb,"roofit"} );
	...
	RooRealVar D0FloatWidth("D0FloatWidth","D0FloatWidth",0,-0.6,0.6);
	RooRealVar D0MassMean("D0MassMean","D0MassMean",1.86483,1.85483,1.87483);
  ...
  if(extraName=="DalphaScan" || extraName=="Dchi2clScan" || extraName == "DdlsScan" || extraName=="PhiMassScan" || extraName=="FixShapeDefault" || extraName=="KaonPtScan" || extraName=="PionPtScan" || extraName=="AllDauPtScan" || extraName=="Reschi2clScan"){
    ...
    D0MassMean.setVal(D0MassMeanV_N);
    DsMassMean.setConstant(kTRUE);
    D0MassMean.setConstant(kTRUE);
		D0FloatWidth.setVal(D0FloatWidthV_N);
  }
	...
	// RooRealVar D0MassMean("D0MassMean","D0MassMean",1.86483,1.85483,1.87483);
	// RooRealVar D0width("D0width","D0width",0.01,0.000001,0.1);
	// RooGaussian GausD0("GausD0","gauss(Dmass,D0MassMean,D0width)",Dmass,D0MassMean,D0width);

	RooFormulaVar scale_width1D0("scale width1D0","scaled width1D0","DsWidth1*(1+D0FloatWidth)",RooArgSet(DsWidth1,D0FloatWidth));
	RooFormulaVar scale_width2D0("scale width2D0","scaled width2D0","DsWidth2*(1+D0FloatWidth)",RooArgSet(DsWidth2,D0FloatWidth));

  RooGaussian GausD01("GausD01","gauss(Dmass,DsMassMean,scale_width1D0)",Dmass,D0MassMean,scale_width1);
  RooGaussian GausD02("GausD02","gauss(Dmass,DsMassMean,scale_width2D0)",Dmass,D0MassMean,scale_width2);
 
	//RooGaussian GausD01("GausD01","gauss(Dmass,DsMassMean,scale_width1)",Dmass,D0MassMean,scale_width1);
  //RooGaussian GausD02("GausD02","gauss(Dmass,DsMassMean,scale_width2)",Dmass,D0MassMean,scale_width2);
  RooAddPdf GausD0("GausD0","GausD0",RooArgList(GausD01,GausD02),DsGaus1Fr);


	D0FloatWidth.setConstant(kTRUE);
...
*/
	...
	RooRealVar NumDp("NumDp","Number of D+",N_sigV*0.5,0,4e7);


	RooAddPdf RooDsMixPdf("RooDsMixPdf","RooDsMixPdf",RooArgList(SigPdf,GausD0,*BkgPdf),RooArgList(NumSig,NumDp,NumBkg));	
...
if(doSigVariation<=1){
	...
	D0FloatWidth.setConstant(kFALSE);
	D0FloatWidth.setVal(DsFloatWidth.getValV());
	D0FloatWidth.setConstant(kTRUE);
	fitresult = RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20),SumW2Error(kTRUE));
  ...
  D0MassMeanV_N=D0MassMean.getValV();
	D0FloatWidthV_N=D0FloatWidth.getValV();
...
*/
  ...
  massframe_pull->Draw();
	...
	double hmax=h_DsMassData->GetMaximum()*(double)nbin_hdata/(double)nbin_DmassDraw * (DsDataFitRangeHigh-DsDataFitRangeLow)/(2.11-1.91);
	double hmin=h_DsMassData->GetMinimum()*(double)nbin_hdata/(double)nbin_DmassDraw * (DsDataFitRangeHigh-DsDataFitRangeLow)/(2.11-1.91);
  ...
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,"|y|<1.0"); shiftY-=oneshift;
	...
	shiftY=0.07;
	shiftX=0.05;
	tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("D^{+} : %.0f #pm %.0f ",NumDp.getValV(),NumDp.getError()  ) );
	...
	TLegend *le_nopull=new TLegend(0.62,0.39,0.85,0.59);
  ...
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,"|y|<1.0"); shiftY-=oneshift;
	...
	TLegend *le_withpull=new TLegend(0.62,0.39,0.85,0.59);
  ...
  massframe_pull->Draw();
	...
	if(h_RawRooFitYieldDp==NULL){
		cout<<"h_RawRooFitYieldDp==NULL"<<endl;
	}

	if(h_RawRooFitYieldDp!=NULL){
		cout<<"h_RawRooFitYieldDp!=NULL"<<endl;
  h_RawRooFitYieldDp->SetBinContent(1,NumDp.getValV());
  h_RawRooFitYieldDp->SetBinError(1,NumDp.getError());
...
int SignalFit_wideRange_DpDsShape(int isPbPb=0, int ibin_Dpt=7,int doMCstudySet=0, int doBkgVariationSet=0, int doSigVariationSet=0, int doCutScanSet=1){
	...
	gSystem->MakeDirectory(Form("output_wide%s",s_CutSet.Data()));
	gSystem->MakeDirectory(Form("output_wide%s/MCstudy",s_CutSet.Data()));
...
//	TString dataName=Form("/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/BuildFitFile_FromDsMinTree/output/DsFitFile_DsMinTree_pp_Data_MBAll.root_0%s.root",s_CutSet.Data());
  TString dataName=Form("/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/BuildFitFile_FromDsMinTree/output/DsFitFile_DsMinTree_pp_Data_MBAll.root_0%s.root",s_CutSet.Data());
	...
	TString outName=Form("output_wide%s/FitResult_pp_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
	TString str_PbPb="pp";

	if(doMCstudySet || SaveRootResult==0){ // avoid overwritten other fit result
		outName=Form("output_wide%s/MCstudy/FitResult_pp_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
	}
  ...
  double *Dchi2clMinScan_bins=Dchi2clMinScan_bins_pp;
	...
	if(isPbPb==3){
	  ...
	  DptHigh = bins_pt[ibin_Dpt+1];
		...
		outName=Form("output_wide%s/FitResult_PbPb3_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
		str_PbPb="PbPb";


	if(doMCstudySet){
		outName=Form("output_wide%s/MCstudy/FitResult_PbPb3_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
	}
	...
	TH1F *h_RawRooFitYieldDp=new TH1F("h_RawRooFitYieldDp","h_RawRooFitYieldDp",1,0,1); h_RawRooFitYieldDp->Sumw2();
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false, TString extraName2=""){
	...
	RooFitResult *Roofitresult = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, "", h_RawRooFitYieldDp);
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false,TString extraName2="")
	...
	h_RawRooFitYieldDp->Write("",TObject::kOverwrite);
