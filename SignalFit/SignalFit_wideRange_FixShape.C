...
//  TLatex* texSimPbPb = new TLatex(0.88,tex_upperY, "530 #mub^{-1} (5.02 TeV PbPb)");
 ...
 // 3. fit
  ...
  if(extraName=="DalphaScan" || extraName=="Dchi2clScan" || extraName == "DdlsScan" || extraName=="PhiMassScan" || extraName=="FixShapeDefault"){
...
*/
	...
	RooRealVar NumD0("NumD0","Number of D0",N_sigV*0.5,-1e2,4e7);


	RooAddPdf RooDsMixPdf("RooDsMixPdf","RooDsMixPdf",RooArgList(SigPdf,GausD0,*BkgPdf),RooArgList(NumSig,NumD0,NumBkg));	
...
int SignalFit_wideRange_FixShape(int isPbPb=3, int ibin_Dpt=4,int doMCstudySet=0, int doBkgVariationSet=0, int doSigVariationSet=0, int doCutScanSet=1){
	...
	TString outName=Form("output_wide%s/FitResult_FixShape_pp_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
	TString str_PbPb="pp";
  ...
  double *Dchi2clMinScan_bins=Dchi2clMinScan_bins_pp;
	...
	if(isPbPb==3){
	  ...
	  DptHigh = bins_pt[ibin_Dpt+1];
		...
		outName=Form("output_wide%s/FitResult_FixShape_PbPb3_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
		str_PbPb="PbPb";
