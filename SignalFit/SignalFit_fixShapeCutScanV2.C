 ...
 // 3. fit
  ...
  // SavePlotDirs(c_roofit_MC[count_c_rootfit],Form("%s_MC_unbinfit_pt%.0fto%.0f_%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data() ),{"SignalFit",str_PbPb,"roofit"} );
	...
	RooAddPdf SigPdf("SigPdf","SigPdf",RooArgList(Gaus1,Gaus2),DsGaus1Fr);
 	...
 	RooRealVar Cheb1("Cheb1","Cheb1",0,-10,10); // no input is better than with input
	RooRealVar Cheb2("Cheb2","Cheb2",0,-10,10);
	RooRealVar Cheb3("Cheb3","Cheb3",0,-10,10);
...
*/
	...
	if(doBkgVariation==1 ){
		if((isPbPb==0 && DptLow>=20)){BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2));
		}else{ BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2,Cheb3));}
  }else{
		if((isPbPb==0 && DptLow>=20)){BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1));
		}else{ BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2));}
  }
...
// /temp0128	RooDsMixPdf.plotOn(massframe,Components(*BkgPdf),LineColor(4),LineStyle(kDashed));
  ...
  hpull=massframe->pullHist();
  //  /temp0128 RooDsMixPdf_exp.plotOn(massframe,Components(expbkg),LineColor(4),LineStyle(kDashed));
	...
	hpull=massframe->pullHist();
// /temp0128 	RooDsMixPdf_1G.plotOn(massframe,Components(*BkgPdf),LineColor(4),LineStyle(kDashed));
   ...
   // RooGoF goftest(massframe->getHist("h_modelData"),massframe->getCurve("model_Norm[Dmass]"));
	 ...
	 if(doSigVariation==2){ s_pdf="RooDsMixPdf_1G_Norm[Dmass]";}	
...
int SignalFit_fixShapeCutScanV2(int isPbPb=0, int ibin_Dpt=4,int doMCstudySet=0, int doBkgVariationSet=1, int doSigVariationSet=1, int doCutScanSet=0){
