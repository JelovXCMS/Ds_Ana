...
// modified , also save default result with fix shape fit as referece point
...
// TCanvas *c_roofit = new TCanvas("c_roofit","c_roofit",800,800);
	...
	TCanvas *c_roofit_MC_withpull[500] ;
	...
	double DsWidth1V_N=0;
	double DsWidth2V_N=0;
	double DsGaus1FrV_N=0;
	double DsFloatWidthV_N=0;
	double DsMassMeanV_N=0;
...
*/
	...
	h_DsMassData->Fit("f1_DsMix","L MN0","",  DsDataFitRangeLow, DsDataFitRangeHigh);
	h_DsMassData->Fit("f1_DsMix","L M IN0","",DsDataFitRangeLow, DsDataFitRangeHigh);
	h_DsMassData->Fit("f1_DsMix","L M I0","",DsDataFitRangeLow, DsDataFitRangeHigh);
...
//	f1_DsBkg->SetParError(3,f1_DsMix->GetParError(9));
	...
	// double yieldBkgTry = f1_DsMix->GetParameter(6)/h_DsMassData->GetBinWidth(1);
  // cout<<"yield Bkg from Para 6 = "<< yieldBkgTry <<endl;
	binFitbkg=yieldBkg; 
  ...
  c_binfit_Data[count_c_binfit]=new TCanvas(Form("c_binfit_Data_%i",count_c_binfit),"c_binfit_Data",800,800);
  ...
  myfun->Draw("same");
	...
	tl_binfitData->DrawLatexNDC(textposx+0.28,textposy+shiftY,Form("%.0f < pt < %.0f ",DptLow,DptHigh)); shiftY-=oneshift;
	if(extraName!="")
	{ 	
	tl_binfitData->DrawLatexNDC(textposx,textposy+shiftY,extraName); shiftY-=oneshift;
	}
	tl_binfitData->DrawLatexNDC(textposx+0.28,textposy+shiftY,Form("Raw Yield = %.1f #pm %.1f ",yieldDsSignal,yieldDsSignalErr ));  shiftY-=oneshift;
...
RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Int_t doMCstudy= 0, TString extraName2=""){
  ...
  TLatex* texCmsPre = new TLatex(0.15,tex_upperY, "#scale[1.25]{#bf{CMS}}");
	if(prelim){
		texCmsPre = new TLatex(0.15,tex_upperY, "#scale[1.25]{#bf{CMS}} #it{Preliminary}");
	}
  texCmsPre->SetNDC();
  ...
  TLatex* texCmsSim = new TLatex(0.15,tex_upperY, "#scale[1.25]{#bf{CMS}} #it{Simulations}");
  texCmsSim->SetNDC();
...
//  TLatex* texSimPbPb = new TLatex(0.88,tex_upperY, "530 #mub^{-1} (5.02 TeV PbPb)");
  ...
  texSimpp->SetTextFont(42);
	...
	if(doBkgVariation>=1){
		extraName+=Form("BkgFunVar%i",doBkgVariation);
		if(doBkgVariation==1) {S_Varaition="Bkg 2nd order poly.";}
		if(doBkgVariation==2) {S_Varaition="Bkg exponential.";}
		if(doBkgVariation==3) {S_Varaition="Bkg 2nd order poly. Sig fix";}
	}

	if(doBkgVariation>=1){
		cout<<"doBkgVariation = "<<doBkgVariation<<endl;
		cout<<"extraName = "<<extraName<<endl;	
	}

	if(doSigVariation>=1){extraName+=Form("SigFunVar%i",doSigVariation);
		if(doSigVariation==1) {S_Varaition="Sig WidthVar";}
		if(doSigVariation==2) {S_Varaition="Sig Single Gaus.";}
		if(doSigVariation==3) {S_Varaition="Sig Single Gaus. BkgFix";}
	}
  ...
  // double N_bkgV = f1_DsMix->GetParameter(6)/h_DsMassData->GetBinWidth(1); // this is wrong
  double N_bkgV = binFitbkg;
  ...
  RooRealVar Ddca("Ddca","Ddca",0,0.1); // temp
	...
	RooDataSet RooDS_MC("RooDS_MC","RooDS_MC",RooArgSet(Dmass,D0DataWeight),WeightVar(D0DataWeight),Import(*t_MC));
	...
	c_roofit_MC[count_c_rootfit] = new TCanvas(Form("c_roofit_MC_%i",count_c_rootfit),"c_roofit_MC",c_wtopx,c_wtopy,c_W,c_H);
	...
	massframe_MC->Draw();	
 	...
 	c_roofit_Data[count_c_rootfit] = new TCanvas(Form("c_roofit_Data_%i",count_c_rootfit),"c_roofit_Data",c_wtopx,c_wtopy,800,800);
 ...
 // 3. fit
	...
	// RooGaussian SigPdf_MC("SigPdf_MC","gauss(Dmass,DsMassMean_MC,DsWidth1_MC)",Dmass,DsMassMean_MC,DsWidth1_MC);
	...
	fitresult_MC = SigPdf_MC.fitTo(RooDS_MC,Save(),Range(1.935,2.005),NumCPU(10),SumW2Error(kTRUE));
	// fitresult_MC = SigPdf_MC.fitTo(RooDS_MC,Save(),NumCPU(10));
	fitresult_MC->Print("v");
	SigPdf_MC.plotOn(massframe_MC,LineColor(2));
	}else if(doSigVariation==2){ 
	Gaus1_MC.fitTo(RooDS_MC,NumCPU(10));
	fitresult_MC = Gaus1_MC.fitTo(RooDS_MC,Save(),Range(1.935,2.005),NumCPU(10),SumW2Error(kTRUE));
	fitresult_MC->Print("v");
	...
	c_roofit_MC[count_c_rootfit]->cd();	
	SetCanvas(c_roofit_MC[count_c_rootfit]);	
  ...
  tl_roofitMC->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("%.0f < p_{T} < %.0f GeV/c",DptLow,DptHigh)); shiftY-=oneshift;
  if(extraName!="" || extraName2!="")
  {
  tl_roofitMC->DrawLatexNDC(textposx+shiftX,textposy+shiftY,S_Varaition); shiftY-=oneshift;
  }
	if(doSigVariation<=1){
  ...
  SavePlotDirs(c_roofit_MC[count_c_rootfit],Form("%s_MC_unbinfit_pt%.0fto%.0f_%s%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data(),s_pre.Data() ),savedirs );
  // SavePlotDirs(c_roofit_MC[count_c_rootfit],Form("%s_MC_unbinfit_pt%.0fto%.0f_%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data() ),{"SignalFit",str_PbPb,"roofit"} );
	...
	c_roofit_MC_withpull[count_c_rootfit] = new TCanvas(Form("c_roofit_MC_withpull_%i",count_c_rootfit),"c_roofit_MC_withpull",c_wtopx,c_wtopy,c_W,800); 
  c_roofit_MC_withpull[count_c_rootfit]->cd();
  c_roofit_MC_withpull[count_c_rootfit]->SetFillColor(0);
  c_roofit_MC_withpull[count_c_rootfit]->SetBorderMode(0);
  c_roofit_MC_withpull[count_c_rootfit]->SetFrameFillStyle(0);
  c_roofit_MC_withpull[count_c_rootfit]->SetFrameBorderMode(0);
  c_roofit_MC_withpull[count_c_rootfit]->SetLeftMargin( c_Lmg );
  c_roofit_MC_withpull[count_c_rootfit]->SetRightMargin( c_Rmg );
  c_roofit_MC_withpull[count_c_rootfit]->SetTopMargin( c_Tmg );
  c_roofit_MC_withpull[count_c_rootfit]->SetBottomMargin( c_Bmg );
  c_roofit_MC_withpull[count_c_rootfit]->SetTickx(0);
  c_roofit_MC_withpull[count_c_rootfit]->SetTicky(0);


  TPad *pad3 = new TPad("pad3","top pad",0.0,0.25,1.0,1.0);
  pad3->SetTopMargin(0.08);
  pad3->SetBottomMargin(0.0);
  pad3->SetRightMargin(c_Rmg);
  pad3->SetLeftMargin(c_Lmg);
  pad3->Draw();
  TPad *pad4 = new TPad("pad4","bottom pad",0.0,0.00,1.0,0.25);
  pad4->SetTopMargin(0.0);
  pad4->SetBottomMargin(0.30);
  pad4->SetRightMargin(c_Rmg);
  pad4->SetLeftMargin(c_Lmg);
  pad4->Draw();
 ...
 pad4->cd();
	...
	RooHist *hpull_MC=NULL;
  hpull_MC=massframe_MC->pullHist();

 RooPlot *massframe_pull_MC=new RooPlot("massframe_pull_MC","Dmass",Dmass,DsDataFitRangeLow,DsDataFitRangeHigh,nbin_DmassDraw);
  massframe_pull_MC->addPlotable(hpull_MC,"P");

	massframe_pull_MC->SetMaximum(4.9);
	massframe_pull_MC->SetMinimum(-4.9);
  massframe_pull_MC->GetXaxis()->SetTitleSize(0.14);
  massframe_pull_MC->GetXaxis()->SetLabelSize(0.11);
  massframe_pull_MC->GetYaxis()->SetTitleSize(0.14);
  massframe_pull_MC->GetYaxis()->SetTitleOffset(0.4);
  massframe_pull_MC->GetYaxis()->SetLabelSize(0.11);
  massframe_pull_MC->Draw();


	pad3->cd();

	 ofstream file_pval_MC;
	 file_pval_MC.open(Form("./output%s/pval%s_MC.out",s_CutSet.Data(),str_PbPb.Data()),std::fstream::in | std::fstream::out | std::fstream::app);




   RooGoF goftest_unbinned_pre_MC(&RooDS_MC,&SigPdf_MC,&Dmass);
   goftest_unbinned_pre_MC.setRange(Dmass.getMin(),Dmass.getMax());
   double pvalue_AD_MC, testStat_AD_MC, pvalue_KS_MC, testStat_KS_MC;
   goftest_unbinned_pre_MC.ADTest(pvalue_AD_MC,testStat_AD_MC);

   // cout << "AD (asym.) pval: " << pvalue_AD << " ,testStat " << testStat_AD << endl;
   // file_pval << "AD (asym.) pval: " << pvalue_AD << " ,testStat " << testStat_AD << endl;
   goftest_unbinned_pre_MC.KSTest(pvalue_KS_MC,testStat_KS_MC);

   // cout << "KS (asym.) pval: " << pvalue_KS << " ,testStat " << testStat_KS << endl;
   // file_pval << "KS (asym.) pval: " << pvalue_KS << " ,testStat " << testStat_KS << endl;

	 massframe_MC->Print("v");
	TString s_pdf_MC="SigPdf_MC_Norm[Dmass]_Range[fit_nll_SigPdf_MC_RooDS_MC]_NormRange[fit_nll_SigPdf_MC_RooDS_MC]";
	// TString s_pdf_MC="SigPdf_MC_Norm[Dmass]";
	if(doSigVariation==2){ 
		s_pdf_MC="Gaus1_MC_Norm[Dmass]";
	}

   RooGoF goftest_MC(massframe_MC->getHist("h_RooDS_MC"),massframe_MC->getCurve(s_pdf_MC));
   cout<<"check 0"<<endl;
   goftest_MC.setRange(Dmass.getMin(),Dmass.getMax());
   goftest_MC.setRebin(5,false); // better rebin to make sure that all bins have >=5 expected events
   int ndf_MC=0;

  cout<<"check 1"<<endl;

   int d_ndf_MC=fitresult_MC->floatParsFinal().getSize();

   cout<<"check 2"<<endl;
   double pvalue_BC_MC=0.0;
   double testStat_BC_MC=0.0;
   goftest_MC.BCChi2Test(pvalue_BC_MC,testStat_BC_MC,ndf_MC,d_ndf_MC);
   file_pval_MC << "BC: " << pvalue_BC_MC << ", " << testStat_BC_MC << endl;
   cout<<"BC: " << pvalue_BC_MC << ", " << testStat_BC_MC << endl;

   cout<<"check 3"<<endl;
   double pvalue_RooFit_MC=0.0;
   double testStat_RooFit_MC=0.0;
   goftest_MC.RooFitChi2Test(pvalue_RooFit_MC,testStat_RooFit_MC,ndf_MC,d_ndf_MC);
   file_pval_MC << "RooFitChi2: " << pvalue_RooFit_MC << ", " << testStat_RooFit_MC << endl;
   cout<<"RooFitChi2: " << pvalue_RooFit_MC << ", " << testStat_RooFit_MC << endl;


  shiftY=-0.25;
  shiftX=0.40;
  // tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("AD pvale : %.2f",pvalue_AD)); shiftY-=oneshift;
  TLatex *tl_roofitMC1 =new TLatex();
  tl_roofitMC1->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("KS pvale : %.1e",pvalue_KS_MC)); shiftY-=oneshift;
  tl_roofitMC1->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("BC pvale : %.1e",pvalue_BC_MC)); shiftY-=oneshift;
  tl_roofitMC1->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("RooFitChi2 pvale : %.1e",pvalue_RooFit_MC)); shiftY-=oneshift;

  SavePlotDirs(c_roofit_MC_withpull[count_c_rootfit],Form("%s_MC_unbinfit_pt%.0fto%.0f_%s%s_withGOF%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data(),s_pre.Data() ),savedirs );
	...
	// RooRealVar DsMassMean("DsMassMean","DsMassMean",DsMassMeanV,1.964,1.972);
	RooRealVar DsMassMean("DsMassMean","DsMassMean",DsMassMeanV,1.964,1.985);
	...
	if(extraName=="DalphaScan" || extraName=="Dchi2clScan" || extraName == "DdlsScan" || extraName=="PhiMassScan" || extraName=="FixShapeDefault" || extraName=="KaonPtScan" || extraName=="PionPtScan" || extraName=="AllDauPtScan" || extraName=="Reschi2clScan"){
		DsWidth1.setVal(DsWidth1V_N);
		DsWidth2.setVal(DsWidth2V_N);
		DsGaus1Fr.setVal(DsGaus1FrV_N);
		DsFloatWidth.setVal(DsFloatWidthV_N);
		DsFloatWidth.setConstant(kTRUE);
		DsMassMean.setVal(DsMassMeanV_N);
	...
	RooGaussian SigPdf("SigPdf","gauss(Dmass,DsMassMean,scale_width1)",Dmass,DsMassMean,scale_width1);

	
 	RooRealVar Cheb1("Cheb1","Cheb1",0,-1,1); // no input is better than with input
	RooRealVar Cheb2("Cheb2","Cheb2",0,-1,1);
	RooRealVar Cheb3("Cheb3","Cheb3",0,-1,1);
	RooRealVar Cheb4("Cheb4","Cheb4",0,-1,1);
...
/*
	if(doBkgVariation==1 ){
		if((isPbPb==0 && DptLow>=20)){BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2));
		}else{ BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1));}
  }else{
		if((isPbPb==0 && DptLow>=20)){BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1));
		}else{ BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2));}
  }
*/

	if(doBkgVariation==1 || doBkgVariation==3 ){
		if((isPbPb && DptHigh<=10)){BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2,Cheb3));
		}else{ BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2));}
  }else{
		if((isPbPb  && DptHigh<=10)){BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2));
		}else{ BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1));}
  }

	if(extraName=="Bkg1stTest"){
		BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1));
	}else if(extraName=="Bkg2ndTest"){
    BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2));
	}else if(extraName=="Bkg3rdTest"){
    BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2,Cheb3));
	}else if(extraName=="Bkg4thTest"){
    BkgPdf=new RooChebychev("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1,Cheb2,Cheb3,Cheb4));
	}	


	RooRealVar tau("tau","tau",-1,-10000,100);
	RooExponential expbkg("expbkg","expbkg",Dmass,tau);



	// RooChebychev BkgPdf("BkgPdf","BkgPdf",Dmass,RooArgList(Cheb1));
	RooRealVar NumSig("NumSig","Number of Signal",N_sigV,-1e2,4e7);
	RooRealVar NumBkg("NumBkg","Number of Background",N_bkgV,0,1e11);
	...
	RooAddPdf RooDsMixPdf_exp("RooDsMixPdf_exp","RooDsMixPdf_exp",RooArgList(SigPdf,expbkg),RooArgList(NumSig,NumBkg));	
...
if(doSigVariation<=1){
	...
	RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
	// RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
	if(doBkgVariation==1){RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));}
	if(doBkgVariation==3){
		// set Sig pdf fix
		DsWidth1.setVal(DsWidth1V_N);
		DsWidth2.setVal(DsWidth2V_N);
	...
	// fitresult = RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),Range("Allrange"),Save(),NumCPU(20),SumW2Error(kTRUE));
	// fitresult = RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20),SumW2Error(kTRUE));
	fitresult = RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20),SumW2Error(kTRUE));
	if(NumSig.getValV()<0){   
		NumSig.setVal(0);
		fitresult = RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20),SumW2Error(kTRUE));
	...
	if(fitresult->edm()>0.1){
    // fitresult = RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20));
    fitresult = RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(10));
		// fitresult = RooDsMixPdf.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20),SumW2Error(kTRUE));
	}
  ...
  RooDsMixPdf.plotOn(massframe,Components(*BkgPdf),LineColor(4),LineStyle(kDashed));
	...
	}else if(doBkgVariation==2){

	cout<<"\n\n doBkgVariation == 2 \n\n"<<endl;

  RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
  RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
  // RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
  // fitresult = RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),Range("Allrange"),Save(),NumCPU(20),SumW2Error(kTRUE));
	if(NumSig.getValV()<0){
  	NumSig.setVal(0);
		fitresult = RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20),SumW2Error(kTRUE));
		fitresult = RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20),SumW2Error(kTRUE));
	}	

  fitresult = RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20));
  // fitresult = RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20));


	if(fitresult->edm()>0.1){
    fitresult = RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(10));
	}

  fitresult->Print("v");

  RooDsMixPdf_exp.plotOn(massframe,Components(expbkg),LineColor(4),LineStyle(kDashed));
  RooDsMixPdf_exp.plotOn(massframe,LineColor(2));
  hpull=massframe->pullHist();

	}

}else if(doSigVariation==2 || doSigVariation==3){
...
//		DsFloatWidth.setVal(DsFloatWidthV_N);
//		DsFloatWidth.setConstant(kTRUE);
		Cheb1.setVal(Cheb1V_N);	
		Cheb2.setVal(Cheb2V_N);	
		Cheb3.setVal(Cheb3V_N);	
		Cheb1.setConstant(kTRUE);	
	...
	RooDsMixPdf_1G.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
	RooDsMixPdf_1G.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
	// RooDsMixPdf_1G.fitTo(*RooDS,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
	// fitresult = RooDsMixPdf_1G.fitTo(*RooDS,Extended(kTRUE),Range("Allrange"),Save(),NumCPU(20),SumW2Error(kTRUE));
	// fitresult = RooDsMixPdf_1G.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(20),SumW2Error(kTRUE));
	...
	if(fitresult->edm()>0.1){
    fitresult = RooDsMixPdf_1G.fitTo(*RooDS,Extended(kTRUE),Save(),NumCPU(10));
	}
	fitresult->Print("v");
  ...
  RooDsMixPdf_1G.plotOn(massframe,Components(*BkgPdf),LineColor(4),LineStyle(kDashed));
	...
	if(extraName=="" && extraName2==""){
	DsWidth1V_N=DsWidth1.getValV();
	DsWidth2V_N=DsWidth2.getValV();
	DsGaus1FrV_N=DsGaus1Fr.getValV();
	DsFloatWidthV_N=DsFloatWidth.getValV();
	DsMassMeanV_N=DsMassMean.getValV();

	Cheb1V_N=Cheb1.getValV();
	Cheb2V_N=Cheb2.getValV();
	Cheb3V_N=Cheb3.getValV();

	}

   // save best fit parameters
   RooArgSet* params = RooDsMixPdf.getParameters(Dmass) ;
   RooArgSet* bestFitParams = (RooArgSet*) params->snapshot() ;

	// compute p-value

	// if(doSigVariation==0 && doBkgVariation ==0 && extraName=="" && extraName2==""){

	 // ofstream file_LLRtest;
	 // file_LLRtest.open(Form("./output%s/LLRtest%s.out",s_CutSet.Data(),str_PbPb.Data()),std::fstream::in | std::fstream::out | std::fstream::app);
	// file_LLRtest<<setprecision(25)<<"minNLL = "<<fitresult->minNll()<<"   "<<extraName<<endl;


	 ofstream file_pval;
	 file_pval.open(Form("./output%s/pval%s.out",s_CutSet.Data(),str_PbPb.Data()),std::fstream::in | std::fstream::out | std::fstream::app);

	 file_pval<<extraName<<" , "<<extraName2<<endl;


   RooGoF goftest_unbinned_pre(RooDS,&RooDsMixPdf,&Dmass);
	 if(doSigVariation==2){ goftest_unbinned_pre._pdf=&RooDsMixPdf_1G;}
   if(doBkgVariation==2){ goftest_unbinned_pre._pdf=&RooDsMixPdf_exp;}
	

   goftest_unbinned_pre.setRange(Dmass.getMin(),Dmass.getMax());
   double pvalue_AD, testStat_AD, pvalue_KS, testStat_KS;
   goftest_unbinned_pre.ADTest(pvalue_AD,testStat_AD);

   cout << "AD (asym.) pval: " << pvalue_AD << " ,testStat " << testStat_AD << endl;
   file_pval << "AD (asym.) pval: " << pvalue_AD << " ,testStat " << testStat_AD << endl;
   goftest_unbinned_pre.KSTest(pvalue_KS,testStat_KS);
   ...
   file_pval << "KS (asym.) pval: " << pvalue_KS << " ,testStat " << testStat_KS << endl;
	...
	cout<<"check -1"<<endl;
	 ...
	 massframe->Print("V");
   // RooGoF goftest(massframe->getHist("h_modelData"),massframe->getCurve("model_Norm[Dmass]"));

	 TString s_pdf="RooDsMixPdf_Norm[Dmass]";
	 if(doSigVariation==2 || doSigVariation==3){ s_pdf="RooDsMixPdf_1G_Norm[Dmass]";}	
	 if(doBkgVariation==2){ s_pdf="RooDsMixPdf_exp_Norm[Dmass]";}

   RooGoF goftest(massframe->getHist("h_RooDSAll"),massframe->getCurve(s_pdf));
	 cout<<"check 0"<<endl;
   ...
   file_pval << "RooFitChi2: " << pvalue_RooFit << ", " << testStat_RooFit << endl;
	 ...
	 ofstream file_LLRtest;
	 file_LLRtest.open(Form("./output%s/LLRtest%s.out",s_CutSet.Data(),str_PbPb.Data()),std::fstream::in | std::fstream::out | std::fstream::app);
	if(extraName=="Bkg1stTest" || extraName=="Bkg2ndTest" || extraName=="Bkg3rdTest" || extraName=="Bkg4thTest"){
		file_LLRtest<<extraName;
  	file_LLRtest << " , BC: " <<setprecision(3)<< pvalue_BC;;
   	file_LLRtest << " , RooFitChi2: " << pvalue_RooFit;
		file_LLRtest << " , NLL = "<<setprecision(20)<<fitresult->minNll()<<endl;
...
//   goftest.NeymanChi2Test(pvalue,testStat,ndf,d_ndf);
   // file_pval << "Neyman: " << pvalue << ", " << testStat << endl;
   // goftest.PearsonChi2Test(pvalue,testStat,ndf,d_ndf);
   // file_pval << "Pearson: " << pvalue << ", " << testStat << endl;
   // goftest.RooFitChi2Test(pvalue,testStat,ndf,d_ndf);
	 // file_pval << "RooFit: " << pvalue << ", " << testStat << endl;		
...
*/
  ...
  RooPlot *massframe_pull=new RooPlot("massframe_pull","Dmass",Dmass,DsDataFitRangeLow,DsDataFitRangeHigh,nbin_DmassDraw);
	...
	massframe_pull->SetMaximum(2.9);
	massframe_pull->SetMinimum(-2.9);
  ...
  massframe_pull->Draw();
	...
	SavePlotDirs(c_roofit_Data_pull[count_c_rootfit],Form("%s_Data_unbinfit_pull_pt%.0fto%.0f_%s%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data(),s_pre.Data() ),savedirs );
	...
	TH1 * htemp=RooDS->createHistogram("htemp",Dmass,Binning(nbin_DmassDraw));
	...
	double newMin=hmin-minExtra*hdiff; if(newMin<=0){ newMin=1 ; }
  ...
  c_roofit_Data[count_c_rootfit]->SetTicky(0);
	...
	massframe->GetYaxis()->SetTitleOffset(1.7);
	massframe->GetYaxis()->SetLabelSize(0.028);
	if(DptHigh<10 && isPbPb){
	massframe->GetYaxis()->SetLabelSize(0.026);
	}	
	massframe->GetXaxis()->SetLabelSize(0.035);
	massframe->GetXaxis()->SetTitleSize(0.040);
	massframe->GetXaxis()->SetTitleOffset(1.2);

	massframe->SetTitle("");
	massframe->GetXaxis()->SetTitle("m_{KK#pi} (GeV/c^{2})");
	massframe->GetXaxis()->CenterTitle();
	massframe->GetYaxis()->SetTitle("Events / 4 MeV/c^{2}");
  ...
  TLatex *tl_roofitData =new TLatex();
	tl_roofitData->SetTextSize(0.038);
  // tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("%s_Data_unbinfit",str_PbPb.Data()));  shiftY-=oneshift;
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("D_{S}^{+} + D_{S}^{-}")); shiftY-=oneshift;
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("%.0f < p_{T} < %.0f GeV/c",DptLow,DptHigh)); shiftY-=oneshift;
  if(extraName!=""|| extraName2!="")
  {
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,S_Varaition); shiftY-=oneshift;
  }
  // tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("Raw Yield = %.0f #pm %.0f",NumSig.getValV(),NumSig.getError() ));  shiftY-=oneshift;
	if(doSigVariation<=1){
  ...
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,"|y|<1.0"); shiftY-=oneshift;
	...
	TLegend *le_nopull=new TLegend(0.51,0.49,0.8,0.70);
	le_nopull->SetBorderSize(0);
	le_nopull->AddEntry(hData,"Data","pl");
	le_nopull->AddEntry(hSig,"Signal + Background","l");
	le_nopull->AddEntry(hBkg,"Background","l");
	le_nopull->SetTextSize(0.036);
	le_nopull->Draw("same");


	SavePlotDirs(c_roofit_Data[count_c_rootfit],Form("%s_Data_unbinfit_nopull_pt%.0fto%.0f_%s%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data(),s_pre.Data() ),savedirs ,"gfc");
  ...
  c_roofit_Data_withpull[count_c_rootfit]->SetTicky(0);
	...
	massframe->GetYaxis()->SetTitleSize(0.045);
	massframe->GetYaxis()->SetTitleOffset(1.5);
	massframe->GetYaxis()->SetLabelSize(0.032);
	if(DptHigh<10 && isPbPb){
	massframe->GetYaxis()->SetLabelSize(0.032);
	}	
	massframe->GetXaxis()->SetLabelSize(0.04);
	massframe->GetXaxis()->SetTitleSize(0.045);
	massframe->GetXaxis()->SetTitleOffset(1.15);
  ...
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,"|y|<1.0"); shiftY-=oneshift;
	...
	TLegend *le_withpull=new TLegend(0.58,0.39,0.8,0.59);
  ...
  massframe_pull->Draw();
	...
	SavePlotDirs(c_roofit_Data_withpull[count_c_rootfit],Form("%s_Data_unbinfit_pt%.0fto%.0f_%s%s%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data(),s_pre.Data() ),savedirs ,"c");
  ...
  shiftY=0.07;
	shiftX=0.06;
  // tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("AD pvale : %.2f",pvalue_AD)); shiftY-=oneshift;
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("KS pvale : %.2f",pvalue_KS)); shiftY-=oneshift;
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("BC pvale : %.2f",pvalue_BC)); shiftY-=oneshift;
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("RooFitChi2 pvale : %.2f",pvalue_RooFit)); shiftY-=oneshift;
	
	SavePlotDirs(c_roofit_Data_withpull[count_c_rootfit],Form("%s_Data_unbinfit_pt%.0fto%.0f_%s%s_withGOF%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data(),extraName2.Data(),s_pre.Data() ),savedirs );
	...
	cout<<"NumSig input  = "<<N_sigV<<endl;
	cout<<"NumBkg input = "<<N_bkgV<<endl;
...
*/
  ...
  gStyle->SetPalette(1) ;
  gStyle->SetOptStat(0) ;
  c_roofit_MCstudy[count_c_rootfit] = new TCanvas(Form("c_roofit_MCstudy_%i",count_c_rootfit),Form("c_roofit_MCstudy_%i",count_c_rootfit),1200,900) ;
	SetCanvas(c_roofit_MCstudy[count_c_rootfit]);
  ...
  double tex_upperY1=0.97;
	double texSize=0.07;

  TLatex* texCmsPre1 = new TLatex(0.15,tex_upperY1, "#scale[1.25]{#bf{CMS}}");
  if(prelim){
          texCmsPre1 = new TLatex(0.15,tex_upperY1, "#scale[1.25]{#bf{CMS}} #it{Preliminary}");
  }
  texCmsPre1->SetNDC();
  texCmsPre1->SetTextAlign(12);
  texCmsPre1->SetTextSize(texSize);
  texCmsPre1->SetTextFont(42);

  TLatex* texCmsSim1 = new TLatex(0.15,tex_upperY1, "#scale[1.25]{#bf{CMS}} Simulations");
  texCmsSim1->SetNDC();
  texCmsSim1->SetTextAlign(12);
  texCmsSim1->SetTextSize(texSize);
  texCmsSim1->SetTextFont(42);


  // TLatex* texColPbPb = new TLatex(0.88,tex_upperY, "PbPb #sqrt{s_{NN}} = 5.02 TeV");
  TLatex* texColPbPb1 = new TLatex(0.97,tex_upperY1, "44 #mub^{-1} (5.02 TeV PbPb)");
  texColPbPb1->SetNDC();
  texColPbPb1->SetTextAlign(32);
  texColPbPb1->SetTextSize(texSize);
  texColPbPb1->SetTextFont(42);

  // TLatex* texColpp = new TLatex(0.88,tex_upperY, "pp #sqrt{s} = 5.02 TeV");
  TLatex* texColpp1 = new TLatex(0.97,tex_upperY1, "38 nb^{-1} (5.02 TeV pp )");
  texColpp1->SetNDC();
  texColpp1->SetTextAlign(32);
  texColpp1->SetTextSize(texSize);
  texColpp1->SetTextFont(42);


	texCmsPre1->Draw("SAME");
	if(str_PbPb=="pp"){
	texColpp1->Draw("SAME");
	}else{
	texColPbPb1->Draw("SAME");
	}	
...
*/
  ...
  c_roofit_MCstudy[count_c_rootfit]->cd(2) ; gPad->SetLeftMargin(0.15) ; frame1->GetYaxis()->SetTitleOffset(1.1) ; frame1->Draw() ;
  c_roofit_MCstudy[count_c_rootfit]->cd(3) ; gPad->SetLeftMargin(0.15) ; frame2->GetYaxis()->SetTitleOffset(1.1) ; frame2->Draw() ;
  c_roofit_MCstudy[count_c_rootfit]->cd(4) ; gPad->SetLeftMargin(0.15) ; frame3->GetYaxis()->SetTitleOffset(1.1) ; frame3->Draw() ;
...
//	c->SaveAs("plots/mcstudy.C");
  ...
  gDirectory->Add(mcstudy) ;
	} // end if(doMCstudy==1 || doMCstudy==2)

	if(doMCstudy==1 || doMCstudy==3){
  ...
  // end mcStudy
   ...
   RooGoF goftest_unbinned(RooDS,&RooDsMixPdf,&Dmass);
   goftest_unbinned.setRange(Dmass.getMin(),Dmass.getMax());
   ...
   goftest_unbinned.setNtoys(100,true,NumCPU(20));
   goftest_unbinned.ADTest(pvalue,testStat);
   ...
   for (int i=0; i<ntoys; i++) {
      ...
      *params = *bestFitParams;
      ...
      RooDataSet *datatoy = RooDsMixPdf.generate(Dmass,1000) ;

      // plot it on tmp frame
      RooPlot* xframetoy = Dmass.frame(Title("toy dataset")) ;
      datatoy->plotOn(xframetoy) ;
      RooDsMixPdf.plotOn(xframetoy) ;

      // do gof before
      // RooGoF goftesttoy(xframetoy->getHist("h_modelData"),xframetoy->getCurve("model_Norm[Dmass]"));
      // goftesttoy.setRange(Dmass.getMin(),Dmass.getMax());
      // goftesttoy.setRebin(5,false);
      // RooGoF goftesttoy_unbinned(datatoy,xframetoy->getCurve("model_Norm[x]"),"x");
      RooGoF goftesttoy_unbinned(datatoy,&RooDsMixPdf,&Dmass);
      goftesttoy_unbinned.setRange(Dmass.getMin(),Dmass.getMax());
      goftesttoy_unbinned.ADTest(pval_AD_before,ts_AD_before);
      goftesttoy_unbinned.KSTest(pval_KS_before,ts_KS_before);

      // do the fit
      RooDsMixPdf.fitTo(*datatoy,NumCPU(20)) ;

      // do gof after
      RooPlot* xframetoy2 = Dmass.frame(Title("toy dataset")) ;
      datatoy->plotOn(xframetoy2) ;
      RooDsMixPdf.plotOn(xframetoy2) ;
      // RooGoF goftesttoy2(xframetoy2->getHist("h_modelData"),xframetoy2->getCurve("model_Norm[Dmass]"));
      // goftesttoy2.setRange(Dmass.getMin(),Dmass.getMax());
      // goftesttoy2.setRebin(5,false);
      // RooGoF goftesttoy2_unbinned(datatoy,xframetoy2->getCurve("model_Norm[x]"),"x");
      RooGoF goftesttoy2_unbinned(datatoy,&RooDsMixPdf,&Dmass);
      RooGoF goftesttoy2_unbinned_t(datatoy,&RooDsMixPdf,&Dmass);
      goftesttoy2_unbinned.setRange(Dmass.getMin(),Dmass.getMax());
      goftesttoy2_unbinned_t.setRange(Dmass.getMin(),Dmass.getMax());
   ...
   RooMsgService::instance().setGlobalKillBelow(oldLevel) ;
	...
	c_roofit_GoF[count_c_rootfit]= new TCanvas(Form("c_roofit_GoF_%i",count_c_rootfit),Form("c_roofit_GoF_%i",count_c_rootfit),2400,1200);
	SetCanvas(c_roofit_GoF[count_c_rootfit]);
	c_roofit_GoF[count_c_rootfit]->Divide(4,2);
	c_roofit_GoF[count_c_rootfit]->cd(1);
   tr->Draw("pval_AD_before");

  c_roofit_GoF[count_c_rootfit]->cd(2);
   tr->Draw("ts_AD_before");
  shiftY=0.07;
	shiftX=0.38;
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("%s",str_PbPb.Data())); shiftY-=oneshift;
  tl_roofitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("%.0f < p_{T} < %.0f GeV/c",DptLow,DptHigh)); shiftY-=oneshift; 

  c_roofit_GoF[count_c_rootfit]->cd(3);
   tr->Draw("pval_KS_before");
  c_roofit_GoF[count_c_rootfit]->cd(4);
   tr->Draw("ts_KS_before");
  c_roofit_GoF[count_c_rootfit]->cd(5);
   tr->Draw("pval_AD_after_toys");
  c_roofit_GoF[count_c_rootfit]->cd(6);
   tr->Draw("ts_AD_after");
  c_roofit_GoF[count_c_rootfit]->cd(7);
   tr->Draw("pval_KS_after_toys");
  c_roofit_GoF[count_c_rootfit]->cd(8);
   tr->Draw("ts_KS_after");

	SavePlotDirs(c_roofit_GoF[count_c_rootfit],Form("%s_Data_unbinfit_pt%.0fto%.0f_GoF",str_PbPb.Data(),DptLow,DptHigh),{"SignalFit",str_PbPb,"roofit","MCStudy"} );
	

	}// end if(doMCstudy==1 || doMCstudy==3)
...
int SignalFit_fixShapeCutScanV3(int isPbPb=3, int ibin_Dpt=2,int doMCstudySet=0, int doBkgVariationSet=0, int doSigVariationSet=0, int doCutScanSet=0){
	...
	int doBkgOrderTest=0;
	Int_t SaveRootResult=0;
	saveAllFormat=false; 
	// if(doBkgVariationSet && doSigVariationSet && doCutScanSet){
		// SaveRootResult=1;
	// }
	...
	gSystem->MakeDirectory(Form("output%s/MCstudy",s_CutSet.Data()));

	// TString dataName=Form("/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/BuildFitFile_FromDsMinTree/output/DsFitFile_DsMinTree_pp_Data_MBAll.root_0%s.root",s_CutSet.Data());
	TString dataName=Form("/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/BuildFitFile_FromDsMinTree/output/FitFile_pp_moreScan%s.root",s_CutSet.Data());
	TString mcName_Prompt=Form("/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/MC_Eff/output%s/MC_eff_pp_Prompt_phikkpi.root",s_CutSet.Data());
	TString mcName_NonPrompt=Form("/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/MC_Eff/output%s/MC_eff_pp_NonPrompt_phikkpi.root",s_CutSet.Data());
	TString outName=Form("output%s/FitResult_FixShapeTrkPtScan_pp_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
	TString str_PbPb="pp";

	if(doMCstudySet || SaveRootResult==0){ // avoid overwritten other fit result
		outName=Form("output%s/MCstudy/FitResult_pp_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
	}
  ...
  double *Dchi2clMinScan_bins=Dchi2clMinScan_bins_pp;
	...
	if(isPbPb==3){
	  ...
	  DptHigh = bins_pt[ibin_Dpt+1];
		...
		//dataName="/scratch/halstead/p/peng43/Ds_phikkpi/BuildFitFile_FromDsMinTree/DsFitFile_PbPb3_Data.root"; //old ?
		// dataName="/scratch/halstead/p/peng43/Ds_phikkpi/BuildFitFile_FromDsMinTree/FitFile_PbPb_Data_sig95.root"; //old ?
		dataName=Form("/scratch/halstead/p/peng43/Ds_phikkpi/BuildFitFile_FromDsMinTree/FitFile_PbPb_Data%s.root",s_CutSet.Data()); //old ?

		// dataName="/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/BuildFitFile_FromDsMinTree/output/FitFile_PbPb3.root";
		mcName_Prompt=Form("/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/MC_Eff/output%s/MC_eff_PbPb3_Prompt_phikkpi.root",s_CutSet.Data());
		mcName_NonPrompt=Form("/home/peng43/work/Project/Ds_PbPb/Ds_PbPbAna/MC_Eff/output%s/MC_eff_PbPb3_NonPrompt_phikkpi.root",s_CutSet.Data());
		outName=Form("output%s/FitResult_FixShapeTrkPtScan_PbPb3_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
		str_PbPb="PbPb";


	if(doMCstudySet || SaveRootResult==0 ){
		outName=Form("output%s/MCstudy/FitResult_PbPb3_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
	}
  ...
  // TH1F *h_RawFitYield_Loose=new TH1F("h_RawFitYield_Loose","h_RawFitYield_Loose",nbin_pt,bins_pt);
	...
	cout<<"th1 = "<<Form("h_DsMassData_pt%.0fto%.0f",DptLow,DptHigh)<<endl;
...
*/
	...
	ofstream file_LLRtest;
	 file_LLRtest.open(Form("./output%s/LLRtest%s.out",s_CutSet.Data(),str_PbPb.Data()),std::fstream::in | std::fstream::out | std::fstream::app);
	 // file_LLRtest<<"isPbPb = "<<isPbPb<<" , Dpt : "<<bins_pt[ibin_Dpt]<<" to "<<bins_pt[ibin_Dpt+1]<<endl;
	 file_LLRtest<<" Dpt : "<<bins_pt[ibin_Dpt]<<" to "<<bins_pt[ibin_Dpt+1]<<endl;


	 ofstream file_pval;
	 file_pval.open(Form("./output%s/pval%s.out",s_CutSet.Data(),str_PbPb.Data()),std::fstream::in | std::fstream::out | std::fstream::app);
	 file_pval<<"isPbPb = "<<isPbPb<<" , Dpt : "<<bins_pt[ibin_Dpt]<<" to "<<bins_pt[ibin_Dpt+1]<<endl;

	TF1 *f1_fitOutput=binnedFit(h_DsMassDsMC_Prompt,h_DsMassData,h_RawBinFitYield,ibin_Dpt,DptLow,DptHigh,str_PbPb);
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false, TString extraName2=""){
	...
	double FloatWidth_DsMass=1.965; TString extraName=""; bool doDcaBinsFit=false; int doMCstudy=doMCstudySet; TString extraName2="";
	...
	file_pval<<"end nominal fit"<<endl;


	cout<<"FloatWidthVal_Norm = "<<FloatWidthVal_Norm<<endl;
	cout<<"FloatWidthErr_Norm = "<<FloatWidthErr_Norm<<endl;

	// do FixShape for Default fit

//	int doBkgVariation=0; int BkgFunction=0; int doSigVariation=0; double FloatWidthVal=0; double FloatWidthErr=0; 
	// double FloatWidth_DsMass=1.965; TString extraName=""; bool doDcaBinsFit=false; int doMCstudy=doMCstudySet; TString extraName2="";
	if(!doMCstudySet){
	extraName="FixShapeDefault"; 
	TH1F *h_RawRooFitYield_FixShape=new TH1F("h_RawRooFitYield_FixShape","h_RawRooFitYield_FixShape",1,0,1); h_RawRooFitYield_FixShape->Sumw2();

	RooFitResult *Roofitresult_fixShape = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_FixShape, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy);

	h_RawRooFitYield_FixShape->Write();
	}

	if(doBkgOrderTest){
	extraName="Bkg1stTest";
	
	TH1F *h_RawRooFitYield_Bkg1stTest=new TH1F("h_RawRooFitYield_Bkg1stTest","h_RawRooFitYield_Bkg1stTest",1,0,1); h_RawRooFitYield_Bkg1stTest->Sumw2();
	RooFitResult *Roofitresult_Bkg1stTest = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_Bkg1stTest, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy);

	extraName="Bkg2ndTest";
	
	TH1F *h_RawRooFitYield_Bkg2ndTest=new TH1F("h_RawRooFitYield_Bkg2ndTest","h_RawRooFitYield_Bkg2ndTest",1,0,1); h_RawRooFitYield_Bkg2ndTest->Sumw2();
	RooFitResult *Roofitresult_Bkg2ndTest = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_Bkg2ndTest, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy);

	extraName="Bkg3rdTest";
	
	TH1F *h_RawRooFitYield_Bkg3rdTest=new TH1F("h_RawRooFitYield_Bkg3rdTest","h_RawRooFitYield_Bkg3rdTest",1,0,1); h_RawRooFitYield_Bkg3rdTest->Sumw2();
	RooFitResult *Roofitresult_Bkg3rdTest = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_Bkg3rdTest, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy);

	extraName="Bkg4thTest";
	
	TH1F *h_RawRooFitYield_Bkg4thTest=new TH1F("h_RawRooFitYield_Bkg4thTest","h_RawRooFitYield_Bkg4thTest",1,0,1); h_RawRooFitYield_Bkg4thTest->Sumw2();
	RooFitResult *Roofitresult_Bkg4thTest = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_Bkg4thTest, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy);

	double NLL1=Roofitresult_Bkg1stTest->minNll();
	double NLL2=Roofitresult_Bkg2ndTest->minNll();
	double NLL3=Roofitresult_Bkg3rdTest->minNll();
	double NLL4=Roofitresult_Bkg4thTest->minNll();

	double chi1_2=2*(NLL1-NLL2);
	double chi1_3=2*(NLL1-NLL3);
	double chi1_4=2*(NLL1-NLL4);

	double chi2_3=2*(NLL2-NLL3);
	double chi2_4=2*(NLL2-NLL4);

	double chi3_4=2*(NLL3-NLL4);
	
	double Prob1_2=TMath::Prob(chi1_2,1);
	double Prob1_3=TMath::Prob(chi1_3,2);
	double Prob1_4=TMath::Prob(chi1_4,3);
	
	double Prob2_3=TMath::Prob(chi2_3,1);
	double Prob2_4=TMath::Prob(chi2_4,2);
	double Prob3_4=TMath::Prob(chi3_4,1);

	// file_LLRtest<<setprecision(20);
	// file_LLRtest<<"NLL1 = "<<NLL1<<endl;
	// file_LLRtest<<"NLL2 = "<<NLL2<<endl;
	// file_LLRtest<<"NLL3 = "<<NLL3<<endl;
	// file_LLRtest<<"NLL4 = "<<NLL4<<endl;
	file_LLRtest<<setprecision(4);
	file_LLRtest<<"Prob1_2 = "<<Prob1_2<<" , Prob1_3 = "<<Prob1_3<<" ,Prob1_4 = "<<Prob1_4<<endl;
	file_LLRtest<<"Prob2_3 = "<<Prob2_3<<" , Prob2_4 = "<<Prob2_4<<" ,Prob3_4 = "<<Prob3_4<<endl;


	// cout<<"hello "<<Roofitresult_Bkg4thTest->minNll()<<endl;
	...
	TH1F *h_RawRooFitYield_sigVar=new TH1F("h_RawRooFitYield_sigVar","h_RawRooFitYield_sigVar",1,0,1); h_RawRooFitYield_sigVar->Sumw2();
	TH1F *h_RawRooFitYield_sigVarBkgFix=new TH1F("h_RawRooFitYield_sigVarBkgFix","h_RawRooFitYield_sigVarBkgFix",1,0,1); h_RawRooFitYield_sigVarBkgFix->Sumw2();

	TH1F *h_RawRooFitYield_sigVar3Gaus=new TH1F("h_RawRooFitYield_sigVar3Gaus","h_RawRooFitYield_sigVar3Gaus",1,0,1); h_RawRooFitYield_sigVar3Gaus->Sumw2();
/*
...
*/
	...
	doBkgVariation=0; BkgFunction=0; doSigVariation=3; FloatWidthVal=FloatWidthVal_Norm; FloatWidthErr= -1*FloatWidthErr_Norm; 
	FloatWidth_DsMass=FloatWidth_DsMass_Norm; extraName="SigGaus1_BkgFix"; doDcaBinsFit=false; doMCstudy=false;
	RooFitResult *Roofitresult_sigVarBkgFix = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_sigVarBkgFix, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass ,extraName, doDcaBinsFit, doMCstudy);


/* 3Guas part not done yet
	doBkgVariation=0; BkgFunction=0; doSigVariation=4; FloatWidthVal=FloatWidthVal_Norm; FloatWidthErr= -1*FloatWidthErr_Norm; 
	FloatWidth_DsMass=FloatWidth_DsMass_Norm; extraName="SigGaus3_"; doDcaBinsFit=false; doMCstudy=false;
	RooFitResult *Roofitresult_sigVar3Gaus = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_sigVar3Gaus, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass ,extraName, doDcaBinsFit, doMCstudy);
*/
	...
	h_RawRooFitYield_sigVar3Gaus->Write("",TObject::kOverwrite);
	...
	TH1F *h_RawRooFitYield_bkgVarExp=new TH1F("h_RawRooFitYield_bkgVarExp","h_RawRooFitYield_bkgVarExp",1,0,1); h_RawRooFitYield_bkgVarExp->Sumw2();
	doBkgVariation=2; BkgFunction=1; doSigVariation=0; FloatWidthVal=0; FloatWidthErr=0; 
	FloatWidth_DsMass=FloatWidth_DsMass_Norm; extraName=""; doDcaBinsFit=false; doMCstudy=false;
	RooFitResult *Roofitresult_bkgVarExp = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_bkgVarExp, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass ,extraName, doDcaBinsFit, doMCstudy);

	TH1F *h_RawRooFitYield_bkgVarFixSig=new TH1F("h_RawRooFitYield_bkgVarFixSig","h_RawRooFitYield_bkgVarFixSig",1,0,1); h_RawRooFitYield_bkgVarFixSig->Sumw2();
	doBkgVariation=3; BkgFunction=1; doSigVariation=0; FloatWidthVal=0; FloatWidthErr=0; 
	FloatWidth_DsMass=FloatWidth_DsMass_Norm; extraName=""; doDcaBinsFit=false; doMCstudy=false;
	RooFitResult *Roofitresult_bkgVarFixSig = unbinnedFit(t_DsMassData, t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_bkgVarFixSig, h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass ,extraName, doDcaBinsFit, doMCstudy);
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false,TString extraName2="")
	...
	// use const mc
	// unbinnedFit(t_DsMassData_DalphaMaxScan[ibin], t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData_DalphaMaxScan[ibin], h_RawRooFitYield_DalphaMaxScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false,TString extraName2="")
	...
	// use const mc
	// unbinnedFit(t_DsMassData_Dchi2clMinScan[ibin], t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData_Dchi2clMinScan[ibin], h_RawRooFitYield_Dchi2clMinScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false,TString extraName2="")
	...
	// use const mc
	// unbinnedFit(t_DsMassData_DdlsMinScan[ibin], t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData_DdlsMinScan[ibin], h_RawRooFitYield_DdlsMinScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false,TString extraName2="")
	...
	// use const mc
	// unbinnedFit(t_DsMassData_PhiMassScan[ibin], t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData_PhiMassScan[ibin], h_RawRooFitYield_PhiMassScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
...
/*
 ...
 cout<<"\n\n do KaonPtScan start "<<endl;

  TH1F *h_DsMassData_KaonPtScan[nbin_DauPtScan];
  TTree *t_DsMassData_KaonPtScan[nbin_DauPtScan];
  TH1F *h_DsMassDsMC_Prompt_KaonPtScan[nbin_DauPtScan];
  TTree *t_DsMassDsMC_Prompt_KaonPtScan[nbin_DauPtScan];
  TH1F *h_DsMassDsMC_NonPrompt_KaonPtScan[nbin_DauPtScan];
  TTree *t_DsMassDsMC_NonPrompt_KaonPtScan[nbin_DauPtScan];

	TH1F *h_RawRooFitYield_KaonPtScan[nbin_DauPtScan];
	TH1F *h_RawBinFitYield_KaonPtScan[nbin_DauPtScan];
	TTree *t_DsMassDsMC_KaonPtScan[nbin_DauPtScan];

  for(int ibin = 0; ibin<nbin_DauPtScan; ibin++){

      h_DsMassDsMC_Prompt_KaonPtScan[ibin]=(TH1F*)f_mc_Prompt->Get(Form("h_DsMass_pt%.0fto%.0f_KaonPt_%i",DptLow,DptHigh,ibin));
			t_DsMassDsMC_Prompt_KaonPtScan[ibin]=(TTree*)f_mc_Prompt->Get(Form("t_DsMass_pt%.0fto%.0f_KaonPt_%i",DptLow,DptHigh,ibin));

      h_DsMassDsMC_NonPrompt_KaonPtScan[ibin]=(TH1F*)f_mc_NonPrompt->Get(Form("h_DsMass_pt%.0fto%.0f_KaonPt_%i",DptLow,DptHigh,ibin));
			t_DsMassDsMC_NonPrompt_KaonPtScan[ibin]=(TTree*)f_mc_NonPrompt->Get(Form("t_DsMass_pt%.0fto%.0f_KaonPt_%i",DptLow,DptHigh,ibin));

			h_DsMassData_KaonPtScan[ibin]=(TH1F*)f_data->Get(Form("h_DsMassData_pt%.0fto%.0f_KaonPt_%d",DptLow,DptHigh,ibin));
			t_DsMassData_KaonPtScan[ibin]=(TTree*)f_data->Get(Form("t_DsMassData_pt%.0fto%.0f_KaonPt_%d",DptLow,DptHigh,ibin));

			f_out->cd();
			h_RawRooFitYield_KaonPtScan[ibin]=new TH1F(Form("h_RawRooFitYield_KaonPtScan_%d",ibin), Form("h_RawRooFitYield_KaonPtScan_%d",ibin),1,0,1);
			h_RawBinFitYield_KaonPtScan[ibin]=new TH1F(Form("h_RawBinFitYield_KaonPtScan_%d",ibin), Form("h_RawBinFitYield_KaonPtScan_%d",ibin),1,0,1);
	// private MC doesn't have enough statistics, temporary use NonPrompt for PbPb , Dpt<6 bins , change back when official MC is ready
	// if(isPbPb==3 && DptHigh<=6)
	if(DptHigh<=6)
	{
		t_DsMassDsMC_KaonPtScan[ibin]=t_DsMassDsMC_NonPrompt_KaonPtScan[ibin];
	}else{
    t_DsMassDsMC_KaonPtScan[ibin]=t_DsMassDsMC_Prompt_KaonPtScan[ibin];
	}

	h_DsMassData_KaonPtScan[ibin]->Rebin(2);
	if (DptLow>=10) {
		h_DsMassData_KaonPtScan[ibin]->Rebin(2);
	}
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false,TString extraName2="")
	...
	doBkgVariation=0; BkgFunction=0; doSigVariation=0; FloatWidthVal=0; FloatWidthErr=0; 
	FloatWidth_DsMass=1.965;  extraName="KaonPtScan"; doDcaBinsFit=false; doMCstudy=0; extraName2=Form("KaonPtScan_%d",ibin);

	binnedFit(h_DsMassDsMC_Prompt_KaonPtScan[ibin],h_DsMassData_KaonPtScan[ibin],h_RawBinFitYield_KaonPtScan[ibin],ibin_Dpt,DptLow,DptHigh,str_PbPb, 0,0,0,0,extraName2); //// saving names has not been set.
	unbinnedFit(t_DsMassData_KaonPtScan[ibin], t_DsMassDsMC_KaonPtScan[ibin], h_DsMassDsMC_Prompt, h_DsMassData_KaonPtScan[ibin], h_RawRooFitYield_KaonPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
	// use const mc
	// unbinnedFit(t_DsMassData_KaonPtScan[ibin], t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData_KaonPtScan[ibin], h_RawRooFitYield_KaonPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
	// unbinnedFit(t_DsMassData_KaonPtScan[ibin], t_DsMassDsMC_KaonPtScan[ibin], h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_KaonPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
 ...
 cout<<"\n\n do PionPtScan start "<<endl;

  TH1F *h_DsMassData_PionPtScan[nbin_DauPtScan];
  TTree *t_DsMassData_PionPtScan[nbin_DauPtScan];
  TH1F *h_DsMassDsMC_Prompt_PionPtScan[nbin_DauPtScan];
  TTree *t_DsMassDsMC_Prompt_PionPtScan[nbin_DauPtScan];
  TH1F *h_DsMassDsMC_NonPrompt_PionPtScan[nbin_DauPtScan];
  TTree *t_DsMassDsMC_NonPrompt_PionPtScan[nbin_DauPtScan];

	TH1F *h_RawRooFitYield_PionPtScan[nbin_DauPtScan];
	TH1F *h_RawBinFitYield_PionPtScan[nbin_DauPtScan];
	TTree *t_DsMassDsMC_PionPtScan[nbin_DauPtScan];

  for(int ibin = 0; ibin<nbin_DauPtScan; ibin++){

      h_DsMassDsMC_Prompt_PionPtScan[ibin]=(TH1F*)f_mc_Prompt->Get(Form("h_DsMass_pt%.0fto%.0f_PionPt_%i",DptLow,DptHigh,ibin));
			t_DsMassDsMC_Prompt_PionPtScan[ibin]=(TTree*)f_mc_Prompt->Get(Form("t_DsMass_pt%.0fto%.0f_PionPt_%i",DptLow,DptHigh,ibin));

      h_DsMassDsMC_NonPrompt_PionPtScan[ibin]=(TH1F*)f_mc_NonPrompt->Get(Form("h_DsMass_pt%.0fto%.0f_PionPt_%i",DptLow,DptHigh,ibin));
			t_DsMassDsMC_NonPrompt_PionPtScan[ibin]=(TTree*)f_mc_NonPrompt->Get(Form("t_DsMass_pt%.0fto%.0f_PionPt_%i",DptLow,DptHigh,ibin));

			h_DsMassData_PionPtScan[ibin]=(TH1F*)f_data->Get(Form("h_DsMassData_pt%.0fto%.0f_PionPt_%d",DptLow,DptHigh,ibin));
			t_DsMassData_PionPtScan[ibin]=(TTree*)f_data->Get(Form("t_DsMassData_pt%.0fto%.0f_PionPt_%d",DptLow,DptHigh,ibin));

			f_out->cd();
			h_RawRooFitYield_PionPtScan[ibin]=new TH1F(Form("h_RawRooFitYield_PionPtScan_%d",ibin), Form("h_RawRooFitYield_PionPtScan_%d",ibin),1,0,1);
			h_RawBinFitYield_PionPtScan[ibin]=new TH1F(Form("h_RawBinFitYield_PionPtScan_%d",ibin), Form("h_RawBinFitYield_PionPtScan_%d",ibin),1,0,1);
	// private MC doesn't have enough statistics, temporary use NonPrompt for PbPb , Dpt<6 bins , change back when official MC is ready
	// if(isPbPb==3 && DptHigh<=6)
	if(DptHigh<=6)
	{
		t_DsMassDsMC_PionPtScan[ibin]=t_DsMassDsMC_NonPrompt_PionPtScan[ibin];
	}else{
    t_DsMassDsMC_PionPtScan[ibin]=t_DsMassDsMC_Prompt_PionPtScan[ibin];
	}

	h_DsMassData_PionPtScan[ibin]->Rebin(2);
	if (DptLow>=10) {
		h_DsMassData_PionPtScan[ibin]->Rebin(2);
	}
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false,TString extraName2="")
	...
	doBkgVariation=0; BkgFunction=0; doSigVariation=0; FloatWidthVal=0; FloatWidthErr=0; 
	FloatWidth_DsMass=1.965;  extraName="PionPtScan"; doDcaBinsFit=false; doMCstudy=0; extraName2=Form("PionPtScan_%d",ibin);

	binnedFit(h_DsMassDsMC_Prompt_PionPtScan[ibin],h_DsMassData_PionPtScan[ibin],h_RawBinFitYield_PionPtScan[ibin],ibin_Dpt,DptLow,DptHigh,str_PbPb, 0,0,0,0,extraName2); //// saving names has not been set.
	unbinnedFit(t_DsMassData_PionPtScan[ibin], t_DsMassDsMC_PionPtScan[ibin], h_DsMassDsMC_Prompt, h_DsMassData_PionPtScan[ibin], h_RawRooFitYield_PionPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
	// use const mc
	// unbinnedFit(t_DsMassData_PionPtScan[ibin], t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData_PionPtScan[ibin], h_RawRooFitYield_PionPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
	// unbinnedFit(t_DsMassData_PionPtScan[ibin], t_DsMassDsMC_PionPtScan[ibin], h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_PionPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
 ...
 cout<<"\n\n do AllDauPtScan start "<<endl;

  TH1F *h_DsMassData_AllDauPtScan[nbin_DauPtScan];
  TTree *t_DsMassData_AllDauPtScan[nbin_DauPtScan];
  TH1F *h_DsMassDsMC_Prompt_AllDauPtScan[nbin_DauPtScan];
  TTree *t_DsMassDsMC_Prompt_AllDauPtScan[nbin_DauPtScan];
  TH1F *h_DsMassDsMC_NonPrompt_AllDauPtScan[nbin_DauPtScan];
  TTree *t_DsMassDsMC_NonPrompt_AllDauPtScan[nbin_DauPtScan];

	TH1F *h_RawRooFitYield_AllDauPtScan[nbin_DauPtScan];
	TH1F *h_RawBinFitYield_AllDauPtScan[nbin_DauPtScan];
	TTree *t_DsMassDsMC_AllDauPtScan[nbin_DauPtScan];

  for(int ibin = 0; ibin<nbin_DauPtScan; ibin++){

      h_DsMassDsMC_Prompt_AllDauPtScan[ibin]=(TH1F*)f_mc_Prompt->Get(Form("h_DsMass_pt%.0fto%.0f_AllDauPt_%i",DptLow,DptHigh,ibin));
			t_DsMassDsMC_Prompt_AllDauPtScan[ibin]=(TTree*)f_mc_Prompt->Get(Form("t_DsMass_pt%.0fto%.0f_AllDauPt_%i",DptLow,DptHigh,ibin));

      h_DsMassDsMC_NonPrompt_AllDauPtScan[ibin]=(TH1F*)f_mc_NonPrompt->Get(Form("h_DsMass_pt%.0fto%.0f_AllDauPt_%i",DptLow,DptHigh,ibin));
			t_DsMassDsMC_NonPrompt_AllDauPtScan[ibin]=(TTree*)f_mc_NonPrompt->Get(Form("t_DsMass_pt%.0fto%.0f_AllDauPt_%i",DptLow,DptHigh,ibin));

			h_DsMassData_AllDauPtScan[ibin]=(TH1F*)f_data->Get(Form("h_DsMassData_pt%.0fto%.0f_AllDauPt_%d",DptLow,DptHigh,ibin));
			t_DsMassData_AllDauPtScan[ibin]=(TTree*)f_data->Get(Form("t_DsMassData_pt%.0fto%.0f_AllDauPt_%d",DptLow,DptHigh,ibin));

			f_out->cd();
			h_RawRooFitYield_AllDauPtScan[ibin]=new TH1F(Form("h_RawRooFitYield_AllDauPtScan_%d",ibin), Form("h_RawRooFitYield_AllDauPtScan_%d",ibin),1,0,1);
			h_RawBinFitYield_AllDauPtScan[ibin]=new TH1F(Form("h_RawBinFitYield_AllDauPtScan_%d",ibin), Form("h_RawBinFitYield_AllDauPtScan_%d",ibin),1,0,1);
	// private MC doesn't have enough statistics, temporary use NonPrompt for PbPb , Dpt<6 bins , change back when official MC is ready
	// if(isPbPb==3 && DptHigh<=6)
	if(DptHigh<=6)
	{
		t_DsMassDsMC_AllDauPtScan[ibin]=t_DsMassDsMC_NonPrompt_AllDauPtScan[ibin];
	}else{
    t_DsMassDsMC_AllDauPtScan[ibin]=t_DsMassDsMC_Prompt_AllDauPtScan[ibin];
	}

	h_DsMassData_AllDauPtScan[ibin]->Rebin(2);
	if (DptLow>=10) {
		h_DsMassData_AllDauPtScan[ibin]->Rebin(2);
	}
...
// RooFitResult *unbinnedFit(TTree* t_Data,TTree* t_MC, TH1F* h_DsMassDsMC, TH1F* h_DsMassData, TH1F* h_RawRooFitYield, TH1F* h_YieldDcabin ,Int_t isPbPb=0, Int_t ibin_Dpt=1, Double_t DptLow=0 ,Double_t DptHigh=10 ,TString str_PbPb="pp", Int_t doBkgVariation=0 ,Int_t BkgFunction=0, Int_t doSigVariation=0,double FloatWidthVal=0 , double FloatWidthErr=0, double FloatWidth_DsMass=1.967, TString extraName="", Bool_t doDcaBinsFit= false , Bool_t doMCstudy= false,TString extraName2="")
	...
	doBkgVariation=0; BkgFunction=0; doSigVariation=0; FloatWidthVal=0; FloatWidthErr=0; 
	FloatWidth_DsMass=1.965;  extraName="AllDauPtScan"; doDcaBinsFit=false; doMCstudy=0; extraName2=Form("AllDauPtScan_%d",ibin);

	binnedFit(h_DsMassDsMC_Prompt_AllDauPtScan[ibin],h_DsMassData_AllDauPtScan[ibin],h_RawBinFitYield_AllDauPtScan[ibin],ibin_Dpt,DptLow,DptHigh,str_PbPb, 0,0,0,0,extraName2); //// saving names has not been set.
	unbinnedFit(t_DsMassData_AllDauPtScan[ibin], t_DsMassDsMC_AllDauPtScan[ibin], h_DsMassDsMC_Prompt, h_DsMassData_AllDauPtScan[ibin], h_RawRooFitYield_AllDauPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
	// use const mc
	// unbinnedFit(t_DsMassData_AllDauPtScan[ibin], t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData_AllDauPtScan[ibin], h_RawRooFitYield_AllDauPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
	// unbinnedFit(t_DsMassData_AllDauPtScan[ibin], t_DsMassDsMC_AllDauPtScan[ibin], h_DsMassDsMC_Prompt, h_DsMassData, h_RawRooFitYield_AllDauPtScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
  ...
  cout<<"\n\n do Reschi2clScan start "<<endl;

  TH1F *h_DsMassData_Reschi2clScan[nbin_Reschi2clScan];
  TTree *t_DsMassData_Reschi2clScan[nbin_Reschi2clScan];
  TH1F *h_DsMassDsMC_Prompt_Reschi2clScan[nbin_Reschi2clScan];
  TTree *t_DsMassDsMC_Prompt_Reschi2clScan[nbin_Reschi2clScan];
  TH1F *h_DsMassDsMC_NonPrompt_Reschi2clScan[nbin_Reschi2clScan];
  TTree *t_DsMassDsMC_NonPrompt_Reschi2clScan[nbin_Reschi2clScan];

  TH1F *h_RawRooFitYield_Reschi2clScan[nbin_Reschi2clScan];
  TH1F *h_RawBinFitYield_Reschi2clScan[nbin_Reschi2clScan];
  TTree *t_DsMassDsMC_Reschi2clScan[nbin_Reschi2clScan];
  for(int ibin = 0; ibin<nbin_Reschi2clScan; ibin++){

      h_DsMassDsMC_Prompt_Reschi2clScan[ibin]=(TH1F*)f_mc_Prompt->Get(Form("h_DsMass_pt%.0fto%.0f_Reschi2cl_%i",DptLow,DptHigh,ibin));
      t_DsMassDsMC_Prompt_Reschi2clScan[ibin]=(TTree*)f_mc_Prompt->Get(Form("t_DsMass_pt%.0fto%.0f_Reschi2cl_%i",DptLow,DptHigh,ibin));

      h_DsMassDsMC_NonPrompt_Reschi2clScan[ibin]=(TH1F*)f_mc_NonPrompt->Get(Form("h_DsMass_pt%.0fto%.0f_Reschi2cl_%i",DptLow,DptHigh,ibin));
      t_DsMassDsMC_NonPrompt_Reschi2clScan[ibin]=(TTree*)f_mc_NonPrompt->Get(Form("t_DsMass_pt%.0fto%.0f_Reschi2cl_%i",DptLow,DptHigh,ibin));

      h_DsMassData_Reschi2clScan[ibin]=(TH1F*)f_data->Get(Form("h_DsMassData_pt%.0fto%.0f_Reschi2cl_%i",DptLow,DptHigh,ibin));
      t_DsMassData_Reschi2clScan[ibin]=(TTree*)f_data->Get(Form("t_DsMassData_pt%.0fto%.0f_Reschi2cl_%i",DptLow,DptHigh,ibin));

      f_out->cd();
      h_RawRooFitYield_Reschi2clScan[ibin]=new TH1F(Form("h_RawRooFitYield_Reschi2clScan_%i",ibin), Form("h_RawRooFitYield_Reschi2clScan_%i",ibin),1,0,1);
      h_RawBinFitYield_Reschi2clScan[ibin]=new TH1F(Form("h_RawBinFitYield_Reschi2clScan_%i",ibin), Form("h_RawBinFitYield_Reschi2clScan_%i",ibin),1,0,1);
  // private MC doesn't have enough statistics, temporary use NonPrompt for PbPb , Dpt<6 bins , change back when official MC is ready
  // if(isPbPb==3 && DptHigh<=6)
  if(DptHigh<=6)
  {
    t_DsMassDsMC_Reschi2clScan[ibin]=t_DsMassDsMC_NonPrompt_Reschi2clScan[ibin];
  }else{
    t_DsMassDsMC_Reschi2clScan[ibin]=t_DsMassDsMC_Prompt_Reschi2clScan[ibin];
  }

  h_DsMassData_Reschi2clScan[ibin]->Rebin(2);
  if (DptLow>=10) {
    h_DsMassData_Reschi2clScan[ibin]->Rebin(2);
  }
  doBkgVariation=0; BkgFunction=0; doSigVariation=0; FloatWidthVal=0; FloatWidthErr=0;
  FloatWidth_DsMass=1.965;  extraName="Reschi2clScan"; doDcaBinsFit=false; doMCstudy=0; extraName2=Form("Reschi2clScan_%i",ibin);

  binnedFit(h_DsMassDsMC_Prompt_Reschi2clScan[ibin],h_DsMassData_Reschi2clScan[ibin],h_RawBinFitYield_Reschi2clScan[ibin],ibin_Dpt,DptLow,DptHigh,str_PbPb, 0,0,0,0,extraName2); //// saving names has not been set.
  unbinnedFit(t_DsMassData_Reschi2clScan[ibin], t_DsMassDsMC_Reschi2clScan[ibin], h_DsMassDsMC_Prompt, h_DsMassData_Reschi2clScan[ibin], h_RawRooFitYield_Reschi2clScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);

  // use const mc
  // unbinnedFit(t_DsMassData_Reschi2clScan[ibin], t_DsMassDsMC, h_DsMassDsMC_Prompt, h_DsMassData_Reschi2clScan[ibin], h_RawRooFitYield_Reschi2clScan[ibin], h_YieldDcabin, isPbPb , ibin_Dpt, DptLow, DptHigh, str_PbPb, doBkgVariation, BkgFunction, doSigVariation, FloatWidthVal, FloatWidthErr , FloatWidth_DsMass , extraName, doDcaBinsFit, doMCstudy, extraName2);
