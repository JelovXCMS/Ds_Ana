...
Double_t fitf_expf(Double_t *x, Double_t *par)
{
   if (TMath::Abs(x[0]) > 1.93 && TMath::Abs(x[0]) < 2.01) {
      TF1::RejectPoint();
      return 0;
	 ...
	 Double_t fitval=par[0]*exp(x[0]*par[1]);
	 return fitval;
...
int TF1FitBkg(TH1F* h_DsMassData,  Int_t ibin_Dpt, Double_t DptLow=0 ,Double_t DptHigh=10, TString str_PbPb="pp"){
	...
	TF1 *expf=new TF1("fitf_expf",fitf_expf,DsDataFitRangeLow,DsDataFitRangeHigh,2);
	expf->SetParameters(h_DsMassData->Integral(),0);
	expf->SetLineColor(kOrange+1);
	expf->SetParNames("Constant","exponential");
	h_DsMassData->Fit("fitf_expf","ME I0 S");
	h_DsMassData->Fit("fitf_expf","ME I0 S");
	h_DsMassData->Fit("fitf_expf","ME I0 S");
	h_DsMassData->Fit("fitf_expf","ME I0 S");
	h_DsMassData->Fit("fitf_expf","ME I0 S");
	...
	int nbin=0;
	for(int i=1; i<=h_DsMassData->GetNbinsX(); i++){
		if(h_DsMassData->GetBinLowEdge(i)<1.93 || h_DsMassData->GetBinLowEdge(i)+h_DsMassData->GetBinWidth(i)  >2.01 ){
			nbin++;
			double BinCenter=h_DsMassData->GetBinCenter(i);
			double BinValue=h_DsMassData->GetBinContent(i);
			double BinError=h_DsMassData->GetBinError(i);

			double BinValue_bkg1=bkg1->Eval(BinCenter);
			double BinValue_bkg2=bkg2->Eval(BinCenter);
			double BinWidth=h_DsMassData->GetBinWidth(i);
		
			chi2_bkg1+=pow(BinValue_bkg1-BinValue,2)/BinError;
			chi2_bkg2+=pow(BinValue_bkg2-BinValue,2)/BinError;
	
			cout<<"BinCenter = "<<BinCenter<<" , BinValue = "<<BinValue<<" , BinValue_bkg1 = "<<BinValue_bkg1<<" , BinWidth = "<<BinWidth<<endl;
	...
	cout<<"\n\nchi2_bkg1 = "<<chi2_bkg1<<" ,prob = "<<TMath::Prob(chi2_bkg1,bkg1->GetNDF())<<endl;
	cout<<"chi2_bkg2 = "<<chi2_bkg2<<" ,prob = "<<TMath::Prob(chi2_bkg1,bkg1->GetNDF())<<endl;
	cout<<"nbin = "<<nbin<<endl;
	...
	bkg2->Draw("SAME");
	expf->SetRange(DsDataFitRangeLow,DsDataFitRangeHigh);
	expf->Draw("SAME");
  ...
  le_TF1FitBkg->AddEntry(bkg2,Form("2nd, %.3e/%i", bkg2->GetChisquare(),bkg2->GetNDF()),"l");
	le_TF1FitBkg->AddEntry(bkg1,Form("exp, %.3e/%i", expf->GetChisquare(),expf->GetNDF()),"l");
	// le_TF1FitBkg->AddEntry(bkg1,Form("1st, %.3e/%i", chi2_bkg1,bkg1->GetNDF()),"l");
  // le_TF1FitBkg->AddEntry(bkg2,Form("2nd, %.3e/%i", chi2_bkg2,bkg2->GetNDF()),"l");
...
if(doSigVariation<=1){
  ...
  RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),NumCPU(20));
  RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),NumCPU(20));
  fitresult = RooDsMixPdf_exp.fitTo(*RooDS,Extended(kTRUE),Range("Allrange"),Save(),NumCPU(20));
...
int SignalFit_bkgTest(int isPbPb=3, int ibin_Dpt=2,bool doMCstudySet=0, int doBkgVariationSet=1, int doSigVariationSet=1, int doCutScanSet=1){
