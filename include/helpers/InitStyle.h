...
//10*fontnumber + precision, 4=helvetica-medium-r-normal  "Arial" , 2=scalable and rotatable hardware fonts , 3=scalable and rotatable hardware fonts. Text size is given in pixels


  gROOT->SetStyle("Plain");
  //  gStyle->SetPalette(1);
  
  gStyle->SetStatFont(nFont); //somehow 43 not work here
  gStyle->SetTitleFont(nFont);
...
//gROOT->ProcessLine(".x betterColors.C");	
   ...
   gStyle->SetOptTitle(1);//if zero get rid of title
...
/*
  ...
  TLatex* texColPbPb = new TLatex(0.96,0.93, "PbPb #sqrt{s_{NN}} = 5.02 TeV");
  texColPbPb->SetNDC();
  texColPbPb->SetTextAlign(32);
  texColPbPb->SetTextSize(0.06);
  texColPbPb->SetTextFont(42);

  TLatex* texColpp = new TLatex(0.96,0.93, "pp #sqrt{s_} = 5.02 TeV");
  texColpp->SetNDC();
