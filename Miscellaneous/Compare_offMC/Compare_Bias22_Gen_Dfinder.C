...
TCanvas *Compare_his(TTree *t_ori,TTree *t_new, TString var, int nbin, double binLow, double binHigh, TCut cut= ""){
	...
	t_ori->Project(Form("h_ori_%i",c_count),var.Data(),cut*"weight");
...
void Compare_Bias22_Gen_Dfinder(int PNP=0){

	InitStyle();

	TFile *f_ori=TFile::Open("Dsfinder_MC_pp_Prompt_phikkpi_pt5p7ph2.root");

	TTree *t_ntDs_ori=(TTree*)f_ori->Get("Dfinder/ntDPhikkpi");
	TTree *t_ntGen_ori=(TTree*)f_ori->Get("Dfinder/ntGen");
	TTree *t_ntHlt_ori=(TTree*)f_ori->Get("hltanalysis/HltTree");
	TTree *t_ntSkim_ori=(TTree*)f_ori->Get("skimanalysis/HltTree");
	TTree *t_ntHi_ori=(TTree*)f_ori->Get("hiEvtAnalyzer/HiTree");
	...
	TFile *f_new=TFile::Open("Dsfinder_MC_pp_Prompt22_phikkpi_pt5p7ph2.root");

	TTree *t_ntDs_new=(TTree*)f_new->Get("Dfinder/ntDPhikkpi");
	TTree *t_ntGen_new=(TTree*)f_new->Get("Dfinder/ntGen");
	TTree *t_ntHlt_new=(TTree*)f_new->Get("hltanalysis/HltTree");
	TTree *t_ntSkim_new=(TTree*)f_new->Get("skimanalysis/HltTree");
	TTree *t_ntHi_new=(TTree*)f_new->Get("hiEvtAnalyzer/HiTree");
...
// efficiency compare
  ...
  TCut cutDTrue="DsGen==23333 && DgencollisionId==0";
  ...
  TCut cutGenTrue="GSignalType==1 && GcollisionId==0 && TMath::Abs(Gy)<1";
...
//  TCut cutGenMass=Form("TMath::Abs(Gmass-%f)<%f",DsMass,DsMassRange); // not stored
  ...
  TH1D *h_GenAlltemp_ori = new TH1D("h_GenAlltemp_ori","h_GenAlltemp_ori",10,DptLow,DptHigh); h_GenAlltemp_ori->Sumw2();
  TH1D *h_GenAcctemp_ori = new TH1D("h_GenAcctemp_ori","h_GenAcctemp_ori",10,DptLow,DptHigh); h_GenAcctemp_ori->Sumw2();
  TH1D *h_RecoNormtemp_ori = new TH1D("h_RecoNormtemp_ori","h_RecoNormtemp_ori",10,DptLow,DptHigh); h_RecoNormtemp_ori->Sumw2();
  ...
  TH1D *h_GenAlltemp_new = new TH1D("h_GenAlltemp_new","h_GenAlltemp_new",10,DptLow,DptHigh); h_GenAlltemp_new->Sumw2();
  TH1D *h_GenAcctemp_new = new TH1D("h_GenAcctemp_new","h_GenAcctemp_new",10,DptLow,DptHigh); h_GenAcctemp_new->Sumw2();
  TH1D *h_RecoNormtemp_new = new TH1D("h_RecoNormtemp_new","h_RecoNormtemp_new",10,DptLow,DptHigh); h_RecoNormtemp_new->Sumw2();
  ...
  h_RecoNormEfftemp_new->Divide(h_RecoNormEfftemp_new,h_GenAlltemp_new,1,1,"B");
	...
	DrawCompare(h_RecoNormEfftemp_ori,h_RecoNormEfftemp_new);
