...
void calculateYieldRatio(TString collsyst="pp", TString varname_="Chi2", Int_t varbins=5, Float_t varmin=0.05, Float_t varmax=0.45, Int_t isLarger_=1, TString infname="hist/D0DcaM")
{
  ...
  for(int i=1;i<varbins+1;i++)
  {
      ...
      else
	{
      ...
      double yieldMCErrPrompt;
      double yieldMCPrompt = hMCPSignal->IntegralAndError(binCutMin, binCutMax, 1, hMCPSignal->GetYaxis()->GetNbins(), 1, hMCPSignal->GetZaxis()->GetNbins(), yieldMCErrPrompt);
      ...
      while(fitStatus)
	{
      ...
      hDataCuts->SetBinError(i, bToDYieldError);
			...
			if(runPrompt){
      hMCCuts->SetBinContent(i, yieldMCPrompt);
      hMCCuts->SetBinError(i, yieldMCErrPrompt);
      hDataCuts->SetBinContent(i, promptDYield);
      // hDataCuts->SetBinError(i, promptDYieldError);
      hDataCuts->SetBinError(i, promptDYield*totalYieldInvMassFitError/totalYieldInvMassFit);
			}
  ...
  for(int i=0;i<varbins;i++)
	...
	double MCLoose=hMCCuts->GetBinContent(iLooseCut);
	double MCLooseErr=hMCCuts->GetBinError(iLooseCut);
	double DataLoose=hDataCuts->GetBinContent(iLooseCut);
	double DataLooseErr=hDataCuts->GetBinError(iLooseCut);
  ...
  for(int i=1;i<varbins+1;i++)
	{
		cout<<"iLooseCut = "<<iLooseCut<<endl;
		if(i==iLooseCut){
			hDaRatio->SetBinContent(i, hDataCuts->GetBinContent(i)/hDataCuts->GetBinContent(iLooseCut) );
			hDaRatio->SetBinError(i, hDataCuts->GetBinError(i) / hDataCuts->GetBinContent(iLooseCut)  );
			cout<<"hello"<<endl;
		}
		if(i!=iLooseCut){
			hDaRatio->SetBinContent(i, hDataCuts->GetBinContent(i)/hDataCuts->GetBinContent(iLooseCut) );
			hDaRatio->SetBinError(i, sqrt( abs( pow(hDataCuts->GetBinError(iLooseCut),2)-pow(hDataCuts->GetBinError(i),2 )  ) ) / hDataCuts->GetBinContent(iLooseCut)  );

		}

			hDoubleRatio->SetBinContent(i, hDaRatio->GetBinContent(i)/hMCRatio->GetBinContent(i) );
			hDoubleRatio->SetBinError(i, hDaRatio->GetBinError(i)/hMCRatio->GetBinContent(i)   );	

		cout<<"ivarbin = "<<i<<" , hDaRatio Content = "<<hDaRatio->GetBinContent(i)<<endl;
