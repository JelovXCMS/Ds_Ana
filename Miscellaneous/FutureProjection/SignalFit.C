...
// }
	...
	RooDataSet *dataHL= RooDsMixPdf.generate(Dmass,( NumSig.getValV()+NumBkg.getValV())*lumi_ratio);

	RooPlot *HLframe= new RooPlot("HLframe","Dmass",Dmass,DsDataFitRangeLow,DsDataFitRangeHigh,nbin_DmassDraw);
	dataHL->plotOn(HLframe);	

	cout<<"HLframe minimum = "<<HLframe->GetMinimum()<<endl;

	NumSig.setVal(NumSig.getValV()*lumi_ratio);
	NumBkg.setVal(NumBkg.getValV()*lumi_ratio);
  RooDsMixPdf.fitTo(*dataHL,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
  // RooDsMixPdf.fitTo(*dataHL,Extended(kTRUE),NumCPU(20),SumW2Error(kTRUE));
  RooDsMixPdf.plotOn(HLframe,LineColor(2));
  RooDsMixPdf.plotOn(HLframe,Components(*BkgPdf),LineColor(4),LineStyle(kDashed));
	...
	double hmax=h_DsMassData->GetMaximum()*(double)nbin_hdata/(double)nbin_DmassDraw*lumi_ratio;
	double hmin=h_DsMassData->GetMinimum()*(double)nbin_hdata/(double)nbin_DmassDraw*lumi_ratio;
	...
	HLframe->SetMaximum(newMax);
	HLframe->SetMinimum(newMin);
	// HLframe->SetMaximum(HLframe->GetMaximum()+ (HLframe->GetMaximum()-HLframe->GetMinimum())*0.5);
	// HLframe->SetMinimum(HLframe->GetMinimum()- (HLframe->GetMaximum()-HLframe->GetMinimum())*0.2);
  ...
  c_test->SetBorderMode(0);
  c_test->SetFrameFillStyle(0);
  c_test->SetFrameBorderMode(0);
  c_test->SetLeftMargin( c_Lmg );
  c_test->SetRightMargin( c_Rmg );
  c_test->SetTopMargin( c_Tmg );
  c_test->SetBottomMargin( c_Bmg );
  c_test->SetTickx(0);
  c_test->SetTicky(0);

  HLframe->SetTitle("");
  HLframe->GetXaxis()->SetTitle("m_{KK#pi} (GeV/c^{2})");
  HLframe->GetXaxis()->CenterTitle();
  // HLframe->GetYaxis()->SetTitle(Form("Events/(%.3f)",h_DsMassData->GetBinWidth(2)));
  HLframe->GetYaxis()->CenterTitle();
  HLframe->Draw();
  ...
  tl_binfitData->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form("Raw Yield = %.0f #pm %.0f ",NumSig.getValV(),NumSig.getError() ));  shiftY-=oneshift;
  ...
  le_binfit->AddEntry(hData,"Data","lp");
  le_binfit->AddEntry(hSig,"Signal+Background","l");
  le_binfit->AddEntry(hBkg,"Background","l");
  le_binfit->Draw("SAME");
  ...
  CMS_lumi(c_test, 12, 10);

	saveAllFormat=true;

  SavePlotDirs(c_test,Form("%s_Data_fit_pt%.0fto%.0f_%s",str_PbPb.Data(),DptLow,DptHigh,extraName.Data() ),{"HLProjection","SignalFit",str_PbPb,"unbinfit"} );
...
int SignalFit(int isPbPb=0, int ibin_Dpt=6,int doMCstudySet=0, int doBkgVariationSet=0, int doSigVariationSet=0, int doCutScanSet=0){
	...
	TString outName=Form("output%s/FitResult_pp_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
	TString str_PbPb="pp";
  ...
  double *Dchi2clMinScan_bins=Dchi2clMinScan_bins_pp;
	...
	if(isPbPb==3){
	  ...
	  DptHigh = bins_pt[ibin_Dpt+1];
		...
		outName=Form("output%s/FitResult_PbPb3_pt%.0fto%.0f.root",s_CutSet.Data(),DptLow,DptHigh);
		str_PbPb="PbPb";
