...
int makeReweight_Ddl(int isPbPb=0, TString var_compare="Ddl", TString var_cut="Dpt" , double var_cutLow=6, double var_cutHigh=10, double bins_var_Low=0, double bins_var_High=1, double bins_var_DrawLow=0, double bins_var_DrawHigh=0.1 ){
  ...
  TString S_MCP=Form("../rootF/%sMC_phiPrompt_fitFile_new.root",Str_PbPb3.Data());
  TString S_MCNP=Form("../rootF/%sMC_phiNonPrompt_fitFile_new.root",Str_PbPb3.Data());
...
//	read ratio histograms
	...
	for(int i=0; i<nbin_varcut; i++){
		f_fit[i]=TFile::Open(Form("../fitout_DdlErrWeight/%s_%s_%s%.0fto%.0f.root",Str_PbPb.Data(),var_compare.Data(),var_cut.Data(),bins_varcut[i]*100,bins_varcut[i+1]*100));
		...
		h_ratio[i]->SetMinimum(-2);
		h_ratio[i]->GetXaxis()->SetRangeUser(0,1);
		h_ratio[i]->Draw();
...
//	double TotalWeight;
	...
	t_fit_MCP->SetBranchAddress("Ddl", &Ddl);

	TFile *f_MCP_new=TFile::Open(Form("../rootF/%sMC_phiPrompt_fitFile_new2.root",Str_PbPb3.Data()),"recreate");
	f_MCP_new->cd();
	TTree *t_fit_MCP_new=(TTree*)t_fit_MCP->CloneTree(0);

	// t_fit_new->Branch("DdlErrWeight",&DdlErrWeight,"DdlErrWeight/F");	
	t_fit_MCP_new->Branch("DdlWeight",&DdlWeight);	
	...
	NorFactor[2]	=1;
	NorFactor[3]	=1;
	NorFactor[0]=1.78111/1.81201;
	NorFactor[1]=2.13961/2.06047;
	NorFactor[2]=1.31111/1.36173;
	NorFactor[3]=3.52482/3.33384;
	...
	for(int i=0; i<nentries;i++){
		...
		DdlWeight=1;
		if(Dpt>DptLowest && Dpt<DptHighest && Ddl >DdlMin && Ddl< DdlMax){
		for(int j=0; j<nbin_varcut; j++){
			if(Dpt>bins_varcut[j] && Dpt<=bins_varcut[j+1]){
				DdlWeight=h_ratio[j]->GetBinContent(h_ratio[j]->FindBin(Ddl))* NorFactor[j];
				if(DdlWeight<0.001){	DdlWeight=0.001;}
				// cout<<"DdlWeight = "<<DdlWeight<<endl;	
				break;
			}
		} // for j<nbin_varcut
		} // if Dpt>2 && <40
...
// check the total integral after weight
	...
	for(int i=0; i<nbin_varcut;i++){
		...
		t_fit_MCP_new->Project(Form("h_before_MCP%i",i),s_test.Data(),Form("DdlErrWeight*TotalWeight*(Dpt>%f && Dpt<%f)",bins_varcut[i],bins_varcut[i+1]));
		t_fit_MCP_new->Project(Form("h_after_MCP%i",i),s_test.Data(),Form("DdlWeight*DdlErrWeight*TotalWeight*(Dpt>%f && Dpt<%f)",bins_varcut[i],bins_varcut[i+1]));
	...
	t_fit_MCNP->SetBranchAddress("Ddl", &Ddl);

	TFile *f_MCNP_new=TFile::Open(Form("../rootF/%sMC_phiNonPrompt_fitFile_new2.root",Str_PbPb3.Data()),"recreate");
	f_MCNP_new->cd();
	TTree *t_fit_MCNP_new=(TTree*)t_fit_MCNP->CloneTree(0);

	// t_fit_new->Branch("DdlWeight",&DdlWeight,"DdlWeight/F");	
	t_fit_MCNP_new->Branch("DdlWeight",&DdlWeight);	
	...
	NorFactor[0]=1.963222/1.78478;
	NorFactor[1]=2.96439/2.74205;
	NorFactor[2]=2.82868/2.8852;
	NorFactor[3]=1.08609/1.12553;
...
*/
	...
	for(int i=0; i<nbin_varcut;i++){
		...
		t_fit_MCNP_new->Project(Form("h_before_MCNP%i",i),s_test.Data(),Form("DdlErrWeight*TotalWeight*(Dpt>%f && Dpt<%f)",bins_varcut[i],bins_varcut[i+1]));
		t_fit_MCNP_new->Project(Form("h_after_MCNP%i",i),s_test.Data(),Form("DdlWeight*DdlErrWeight*TotalWeight*(Dpt>%f && Dpt<%f)",bins_varcut[i],bins_varcut[i+1]));
...
int main(int argc, char*argv[]){
  ...
  if(argc==6){
    makeReweight_Ddl(atoi(argv[1]), argv[2], argv[3], atof(argv[4]), atof(argv[5]) );
  }else if(argc==10){
    makeReweight_Ddl(atoi(argv[1]), argv[2], argv[3], atof(argv[4]), atof(argv[5]) , atof(argv[6]) , atof(argv[7]) , atof(argv[8]) , atof(argv[9]));
  }else{
     makeReweight_Ddl();
