...
int Fit_sideband_DdlErrWeight(int isPbPb=0, TString var_compare="DdlErr", TString var_cut="Dpt" , double var_cutLow=6, double var_cutHigh=8, double bins_var_Low=0, double bins_var_High=1, double bins_var_DrawLow=0, double bins_var_DrawHigh=0.1 ){

	// InitStyle();

	gSystem->Exec("mkdir -p fitout_DdlErrWeight");
	gSystem->Exec("mkdir -p plots/fit");
	gSystem->Exec(Form("mkdir -p plots/%s_DdlErrWeight",var_compare.Data()));
  ...
  TString mcName_Prompt="./rootF/ppMC_phiPrompt_fitFile_new.root";
  TString mcName_NonPrompt="./rootF/ppMC_phiNonPrompt_fitFile_new.root";
  ...
  TTree *t_Ds_Data=(TTree*)f_data->Get(Form("t_fit"));
	...
	TFile *f_out=TFile::Open(Form("./fitout_DdlErrWeight/%s_%s_%s%.0fto%.0f.root",Str_PbPb.Data(), var_compare.Data(), var_cut.Data(), var_cutLow*100, var_cutHigh*100) ,"recreate");
...
//	tltx->DrawLatexNDC(textposx+shiftX,textposy+shiftY,Form(""))
	...
	c_var_Data->SaveAs(Form("./plots/%s_DdlErrWeight/%s_DataAndSideBand_%s%.0fto%.0f.png", var_compare.Data(),Str_PbPb.Data(), var_cut.Data(),var_cutLow*100,var_cutHigh*100));
...
*/
	...
	t_Ds_MCPrompt->Project(Form("h_%s_PromptMC",var_compare.Data()),var_compare.Data(),Form("(Dmass>%f && Dmass<%f && %s)*TotalWeight*DdlErrWeight",Dmass2SigLow, Dmass2SigHigh, DataCuts.Data() ));
	...
	t_Ds_MCNonPrompt->Project(Form("h_%s_NonPromptMC",var_compare.Data()),var_compare.Data(),Form("(Dmass>%f && Dmass<%f && %s)*TotalWeight*DdlErrWeight", Dmass2SigLow, Dmass2SigHigh, DataCuts.Data()));
...
//		TH1D *h_eff_nonprompt_f0=(TH1D*)f_mc_NonPrompt_f0->Get(Form("h_RecoEff_for%s",var_compare.Data()));
		...
		t_Ds_MCNonPrompt->Project("h_eff_nonprompt_phi","Dpt",(TCut)Form("TotalWeight*DdlErrWeight*(%s)",DataCuts.Data()));
...
//		cout<<"LumiNevt = "<<LumiNevt<<endl;
	...
	c_var_compare->SaveAs(Form("./plots/%s_DdlErrWeight/%s_%s%.0fto%.0f.png", var_compare.Data(),Str_PbPb.Data(), var_cut.Data(),var_cutLow*100,var_cutHigh*100));
...
int main(int argc, char*argv[]){
	...
	if(argc==6){
		Fit_sideband_DdlErrWeight(atoi(argv[1]), argv[2], argv[3], atof(argv[4]), atof(argv[5]) );
	}else if(argc==10){	
		Fit_sideband_DdlErrWeight(atoi(argv[1]), argv[2], argv[3], atof(argv[4]), atof(argv[5]) , atof(argv[6]) , atof(argv[7]) , atof(argv[8]) , atof(argv[9]));
	}else{
		Fit_sideband_DdlErrWeight();
