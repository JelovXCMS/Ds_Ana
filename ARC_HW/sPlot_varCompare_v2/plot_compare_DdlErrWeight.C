...
int plot_compare_DdlErrWeight(int isPbPb=0, TString var_compare="DdxyzErr", TString var_cut="Dtrk1Pt"){
	...
	for(int i=0; i<nbin_varcut; i++){
		...
		fin[i]=TFile::Open(Form("./fitout_DdlErrWeight/%s_%s_%s%.0fto%.0f.root",Str_ppPbPb.Data(),var_compare.Data(),var_cut.Data(), bins_varcut[i]*100, bins_varcut[i+1]*100 ),"READ");
	...
	c_var->SaveAs(Form("./plots/%s_DdlErrWeight/%s_%s_compare.png",var_compare.Data(),Str_ppPbPb.Data(),var_cut.Data()));
	...
	le->AddEntry(h_NonPromptMC_var,"NonPrompt MC","pl");
	le->Draw("same");
	c_var->SaveAs(Form("./plots/%s_DdlErrWeight/%s_%s_compareWithPNP.png",var_compare.Data(),Str_ppPbPb.Data(),var_cut.Data()));
...
int main(int argc, char *argv[]){
	...
	if(argc==4){
		plot_compare_DdlErrWeight(atoi(argv[1]), argv[2], argv[3] );
	}else{
