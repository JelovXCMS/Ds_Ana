...
using namespace std;
  ...
  double FloatWidth_DsMass_Norm=1.965;
	...
	double DmassSideBandLow=1.93;
	double DmassSideBandHigh=2.01;
...
int Fit_sideband(int isPbPb=0, TString var_compare="DdxyzErr", TString var_cut="Dtrk1Pt" , double var_cutLow=0.75, double var_cutHigh=1.25){
  ...
  TString mcName_NonPrompt_f0="./rootF/ppMC_f0NonPrompt_fitFile";
  ...
  double Dpt_High=Dpt_Hight_pp;
    ...
    dataName="./rootF/PbPb_fitFile.root";
    mcName_Prompt="./rootF/PbPbMC_phiPrompt_fitFile.root";
    mcName_NonPrompt="./rootF/PbPbMC_phiNonPrompt_fitFile.root";
    mcName_NonPrompt_f0="./rootF/PbPbMC_f0NonPrompt_fitFile";
  ...
  massframe->Draw();
	...
	c_MC->SaveAs(Form("./plots/fit/%s_Data_%s%.0fto%.0f.png",Str_PbPb.Data(),var_cut.Data(),var_cutLow*100,var_cutHigh*100));
...
// end fitting
	...
	t_Ds_Data->Project(Form("h_%s_sideband",var_compare.Data()),var_compare.Data(),Form("Dmass<%f || Dmass>%f",DmassSideBandLow, DmassSideBandHigh));
	...
	t_Ds_Data->Project(Form("h_%s_Data2sig",var_compare.Data()),var_compare.Data(),Form("Dmass>%f && Dmass<%f",Dmass2SigLow,Dmass2SigHigh));
...
*/
	...
	t_Ds_MCPrompt->Project(Form("h_%s_PromptMC",var_compare.Data()),var_compare.Data(),"(Dmass>1.949 && Dmass<1.989)*TotalWeight");
	...
	t_Ds_MCNonPrompt->Project(Form("h_%s_NonPromptMC",var_compare.Data()),var_compare.Data(),"(Dmass>1.949 && Dmass<1.989)*TotalWeight");
...
//		TH1D *h_eff_nonprompt_f0=(TH1D*)f_mc_NonPrompt_f0->Get(Form("h_RecoEff_for%s",var_compare.Data()));
		...
		t_Ds_MCNonPrompt->Project("h_eff_nonprompt_phi","Dpt",(TCut)Form("TotalWeight*(%s)",DataCuts.Data()));
...
//		cout<<"LumiNevt = "<<LumiNevt<<endl;
	...
	TH1D *h_var_MixMC= new TH1D("h_var_MixMC","h_var_MixMC",nbin_var, bins_var_Low,bins_var_High); h_var_MixMC->Sumw2();
	...
	TCanvas *c_var_compare= new TCanvas("c_var_compare","c_var_compare");
	c_var_compare->cd();
	h_var_Data2sig->GetXaxis()->SetRangeUser(0,0.1); // normalized before setRange
	h_var_Data2sig->GetXaxis()->SetTitle("DdxyzErr");
	...
	TLegend *le_var = new TLegend(0.5,0.20,0.85,0.65);
