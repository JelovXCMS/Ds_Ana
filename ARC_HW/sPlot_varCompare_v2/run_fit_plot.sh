...
# for (( i=0; i<5 ; i++   ))
	do
  	...
  	./Fit_sideband.exe ${isPbPb} ${var_compare} ${var_cut[${j}]} ${var_cutbins[($i)]} ${var_cutbins[($i+1)]} ${var_cp_Low} ${var_cp_High} ${var_cp_DrawLow} ${var_cp_DrawHigh} > log/Fit_sideband_pPb${isPbPb}_${var_compare}_${var_cut[${j}]}_${var_cutbins[($i)]}_${var_cutbins[($i+1)]}.log
	done
