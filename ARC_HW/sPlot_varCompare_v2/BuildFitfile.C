...
int BuildFitfile(Int_t isPbPb=3, Int_t isReal=1,int PNPrompt=0,int DsChannel=0, TString s_fin="",TString foutTag="", double Dpt_Low=0, double Dpt_High=0){
  ...
  if(isPbPb){
	...
	Float_t DmassLow=1.78;
	Float_t DmassHigh=2.15;
	Float_t PhiMassLow=1.0105;
	Float_t PhiMassHigh=1.0285;
  ...
  TString Str_isPbPb="pp";
	...
	gSystem->Exec("mkdir -p rootF");

	TString fout_name=Form("./rootF/%s%s_fitFile.root",Str_isPbPb.Data(),foutTag.Data());
  ...
  if(!isPbPb && !REAL){
	...
	if(!REAL){
		...
		t_fit->Branch("D0Weight",&D0Weight);
	}
...
//	return 1;
    ...
    DlxyBSs=DsData.DlxyBSs;
		...
		D0Weight=weight*DgenptSampleWeight*RecoFONLLWeight*RecoD0DataWeight;
		if(isPbPb){
			TotalWeight=PbPbVzWeight*DsData.Ncoll * weight*DgenptSampleWeight*RecoFONLLWeight*RecoD0DataWeight*RecoDsDataWeight;
			D0Weight=PbPbVzWeight*DsData.Ncoll * weight*DgenptSampleWeight*RecoFONLLWeight*RecoD0DataWeight;
		}
		...
		// if(DsData.Dpt>Dpt_Low && DsData.Dpt < Dpt_High && DsData.Dmass>DmassLow && DsData.Dmass<DmassHigh && DsData.DtktkResmass>PhiMassLow && DsData.DtktkResmass < PhiMassHigh && Dalpha<Dalpha_cut && Dchi2cl>Dchi2cl_cut && Ddls > Ddls_cut){
...
// ntGen part
	...
	if(!REAL){
		...
		TH1D *hGen_pt_D0Weight=new TH1D("hGen_pt_D0Weight","; Gpt",nbin_pt,bins_pt); hGen_pt_D0Weight->Sumw2();
		ntGen->Project("hGen_pt_D0Weight","Gpt",(TCut)((cutGenTrue && cutGenPNPrompt && cuthiBin && cutGpt)*GenWeight_D0Data.Data()) );	
		fout->cd();
		hGen_pt_D0Weight->Write();
