...
int Plot_DdlScale(int isPbPb=0,TString var_scan="Ddls", int useReweight=0, TString ReweightTree="DdxyzErrWeight"){
  ...
  InitStyle();
	...
	{
		s_fin=Form("./RatioOut/%s%s/%s_%s_Dpt%.0fto%.0f%s%s.root",var_scan.Data(),s_weight.Data(),s_ppPbPb.Data(),var_scan.Data(),bins_pt[i],bins_pt[i+1],s_FixShape.Data(),s_PFrScan.Data());
	...
	le->AddEntry(gr_ScaleBestPfr[nbin_pt-1],Form("Best Fr.%s",var_scan.Data()), "p");
	if(useReweight){
		le->AddEntry((TObject*)0,ReweightTree.Data(), "");
	}
	le->Draw("same");

	gSystem->Exec(Form("mkdir -p Ratioplots_smear/%s%s",var_scan.Data(),s_weight.Data()));
	c_fr->SaveAs(Form("./Ratioplots_smear/%s%s/%s_%s_%s%s_BestScalePfr.png",var_scan.Data(),s_weight.Data(),s_ppPbPb.Data(),var_scan.Data(),s_FixShape.Data(),s_PFrScan.Data()));
	...
	htemp1->SetMaximum(2.5);
	htemp1->SetMinimum(0.4);
	...
	for(int i=startBin; i<nbin_pt ; i++)
	{
		...
		gr_ScaleBest[i]=(TGraphAsymmErrors*)fin[i]->Get("gr_ScaleBest");
		gr_ScaleBest[i]->Draw("p");
	}

	gSystem->Exec(Form("mkdir -p Ratioplots_smear/%s%s",var_scan.Data(),s_weight.Data()));
	c_scl->SaveAs(Form("./Ratioplots_smear/%s%s/%s_%s_%s%s_BestScale.png",var_scan.Data(),s_weight.Data(),s_ppPbPb.Data(),var_scan.Data(),s_FixShape.Data(),s_PFrScan.Data()));
