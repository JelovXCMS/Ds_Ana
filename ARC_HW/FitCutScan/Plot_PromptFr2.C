...
int Plot_PromptFr2(int isPbPb=0,TString var_scan="Ddls", int useReweight=0, TString ReweightTree="DdxyzErrWeight"){
  ...
  InitStyle();
	...
	TGraphErrors *gr_ScaleBest[nbin_pt];
...
*/	
	...
	for(int i=startBin; i<nbin_pt ; i++)
	{
		...
		gr_ScaleBest[i]=(TGraphErrors*)fin[i]->Get("gr_Best_Scale");
