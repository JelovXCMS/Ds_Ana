...
int Plot_PromptFr(int isPbPb=0,TString var_scan="Ddls", int useReweight=0, TString ReweightTree="DdxyzErrWeight"){
  ...
  InitStyle();
	...
	le->AddEntry(gr_PFrBest[nbin_pt-1],Form("Best Fr.%s",var_scan.Data()), "p");
	if(useReweight){
		le->AddEntry((TObject*)0,ReweightTree.Data(), "");
	}
	le->Draw("same");

	gSystem->Exec(Form("mkdir -p RatioScanPlots/%s%s",var_scan.Data(),s_weight.Data()));
	c_fr->SaveAs(Form("./RatioScanPlots/%s%s/%s_%s_%s%s.png",var_scan.Data(),s_weight.Data(),s_ppPbPb.Data(),var_scan.Data(),s_FixShape.Data(),s_PFrScan.Data()));
