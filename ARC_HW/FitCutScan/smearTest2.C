...
int smearTree(TFile *f_mcp_new,TTree *t_mcp, int nSmear=10){
...
int smearTest2(int isPbPb=0,int PNP=0,int nSmear=10){
  ...
  TString mcName=Form("/scratch/halstead/p/peng43/Ds_phikkpi/FitFile_cutScan/%sMC_phi%s_fitFile.root",s_PbPb3.Data(),s_PNP.Data());
	...
	f_mcp_new->cd();
	smearTree(f_mcp_new,t_mcp,nSmear);
	// TTree *t_new=smearTree(t_mcp,nSmear);
	// t_new->Write("",TObject::kOverwrite)
