...
rm CompAndTune_DataMC_Ddls.exe
g++ CompAndTune_DataMC_Ddls.C $(root-config --cflags --libs)  -Wall -O2 -o CompAndTune_DataMC_Ddls.exe
...
# for (( i=0; i<5 ; i++   ))
	do
  	...
  	##		./Fit_splot_onlySave.exe ${isPbPb} ${var_compare} ${var_cut[${j}]} ${var_cutbins[($i)]} ${var_cutbins[($i+1)]} ${var_cp_Low} ${var_cp_High} ${var_cp_DrawLow} ${var_cp_DrawHigh} > log/Fit_splot_pPb${isPbPb}_${var_compare}_${var_cut[${j}]}_${var_cutbins[($i)]}_${var_cutbins[($i+1)]}_saveonly.log &
		...
		if [ $doCompWt -eq 1 ]
			then
  		./CompAndTune_DataMC_Ddls.exe ${isPbPb} ${var_compare} ${var_cut[${j}]} ${var_cutbins[($i)]} ${var_cutbins[($i+1)]} > log/CompAndTune_Ddls_DataMC_pPb${isPbPb}_${var_compare}_${var_cut[${j}]}_${var_cutbins[($i)]}_${var_cutbins[($i+1)]}.log
		fi
	done
		# ./plot_compare.exe ${isPbPb} ${var_compare} ${var_cut[${j}]}
done
