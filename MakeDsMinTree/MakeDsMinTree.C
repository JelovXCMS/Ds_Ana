...
int MakeDsMinTree(TString infile="", TString outfile="", Bool_t REAL=false, Int_t isPbPb=1, Int_t startEntries=0, Int_t endEntries=-1, Bool_t skim=true, Bool_t gskim=true, Bool_t checkMatching=true, Bool_t iseos=false, Bool_t SkimHLTtree=false)
{
  ...
  TTree *ntGen=t_Gen->CloneTree(0);
	...
	nt_Ds->Branch("Dalpha_BS_2D",               &dalpha_BS_2D             );   
	nt_Ds->Branch("Ddls",                       &ddls                     );   
	nt_Ds->Branch("Ddl",                        &ddl                      );   
	nt_Ds->Branch("DdlErr",                     &ddlErr                     );   
	nt_Ds->Branch("DlxyBS",                     &dlxyBS                      );   
	nt_Ds->Branch("DlxyBSErr",                  &dlxyBSErr                      );   
	nt_Ds->Branch("DlxyBSs",                    &dlxyBSs                      );   
  ...
  nt_Ds->Branch("DsGen",&dsGen); // add for using TMVA 
	...
	// nt_Ds->Branch("Dd0Err",                     &dd0Err                    );    
	nt_Ds->Branch("DdxyzErr",                   &ddxyzErr                    );    

	nt_Ds->Branch("DvtxX",                      &dvtxX                    );    
	nt_Ds->Branch("DvtxY",                      &dvtxY                    );    
	nt_Ds->Branch("DvtxZ",                      &dvtxZ                    );    
	nt_Ds->Branch("DvtxXErr",                   &dvtxXErr                    );    
	nt_Ds->Branch("DvtxYErr",                   &dvtxYErr                    );    
	nt_Ds->Branch("DvtxZErr",                   &dvtxZErr                    );    

	nt_Ds->Branch("PVxE",                      &PVxE                    );    
	nt_Ds->Branch("PVyE",                      &PVyE                    );    
	nt_Ds->Branch("PVzE",                      &PVzE                    );    
...
//		nt_Ds->Branch("DgenBAncestorpt",&dgenBAncestorpt);
		...
		nt_Ds->Branch("DgendecayvtxXtoPv", &dgendecayvtxXtoPv);
		nt_Ds->Branch("DgendecayvtxYtoPv", &dgendecayvtxYtoPv);
		nt_Ds->Branch("DgendecayvtxZtoPv", &dgendecayvtxZtoPv);
	...
	*/
	...
	float DsvpvDisSnCut=1.5; // min : pp 1.5, PbPb 2.5
	float DyCut=1;
	float Dchi2clCut=0.02; // min 0.02 pp, PbPb 0.05
	float DalphaCut=0.25; // min 0.2 ?
	if(!REAL){
		DsvpvDisSnCut=0;
		Dchi2clCut=0;
		DalphaCut=3.2;
...
*/
		...
		for(int j=0; j<Dsize ; j++){
			// if(skim){
				...
				if(!REAL && DsGen[j]<=0) continue;  
				if(!REAL && DgencollisionId[j]!=0) continue;  
			...
			dchi2cl=Dchi2cl[j];  
			dalpha=Dalpha[j];
			dalpha_BS_2D=Dalpha_BS_2D[j];
			ddls=DsvpvDistance[j]/DsvpvDisErr[j];
			ddl=DsvpvDistance[j];
			ddlErr=DsvpvDisErr[j];

			dlxyBS=DlxyBS[j];
			dlxyBSs=DlxyBS[j]/DlxyBSErr[j];
			dlxyBSErr=DlxyBSErr[j];
			...
			if(!REAL){
				...
				dgendecayvtxXtoPv =DgendecayvtxXtoPv[j]; 
				dgendecayvtxYtoPv =DgendecayvtxYtoPv[j]; 
				dgendecayvtxZtoPv =DgendecayvtxZtoPv[j]; 
			}
...
int main(int argc, char *argv[])
{
	if(argc==3)
	{
		MakeDsMinTree(argv[1], argv[2]);
	}
	else if(argc==5)
	{
		cout<<"real = "<<atoi(argv[3])<<" ,isPbPb = "<<atoi(argv[4])<<endl;
		MakeDsMinTree(argv[1], argv[2],atoi(argv[3]), atoi(argv[4]));
	}
	else
	{
